...
market-data gap の影響対象は、DB の control row と transaction-local token で作成・終端処理と直列化して固定します。gap work は canonical source identity ごとに exactly-once で作成され、1件の active generation と FIFO queue、enqueue 時点の birth upper bound、immutable member、terminal journal を使って復旧します。導入前の `birth_sequence IS NULL` は `PRE_C` として対象に含め、既存履歴を backfill しません。復旧は page 100 / pass 1,000 の境界で進み、整合性を確定できない work は `UNKNOWN` のまま残して strategy KPI に混ぜません。

...
`HARD_HALT` は durable な cleanup evidence を `UNKNOWN` / `SAFE` で保持します。新しい halt は `UNKNOWN` から始まり、WebSocket loop の起動前、接続失敗・backoff、market event、periodic maintenance で global trading lock 下の `ALL_OPEN_RISK` readback / cleanup を再試行します。価格がなくても flat または order-only なら `SAFE` へ収束します。position close に使う REST ticker は exchange source timestamp の parse 成功、5秒以内の鮮度、未来方向5秒以内の clock skew を必要とし、満たさない場合は execution mutation を作らず `HARD_HALT / UNKNOWN` を維持します。realtime trade event は causal event のため、この REST freshness gate を追加で適用しません。手動 resume は `SAFE` と同一 transaction 内の zero-open-risk readback の両方を必要とし、stale `SAFE` を検出した場合は `UNKNOWN` へ戻して拒否します。

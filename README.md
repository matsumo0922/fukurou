...
保存済みLLM audit graphには、decision単位のbounded reconstruction、14日構造coverage集計、30日経過rootのbounded pruneを行う内部foundationがあります。terminal evidence captureはcode-owned falseのままで、maintenance workerもruntimeへ配線されていないため、reconstruction/coverage/pruneはproductionで実行されず、evidence rowとactivation boundary rowを生成しません。

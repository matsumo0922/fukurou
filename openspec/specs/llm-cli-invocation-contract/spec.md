...
- **THEN** it reads `<manifest-directory>/<manifest-id>.json`, applies the diagnostic manifest validation, and starts without requiring any inherited file descriptor for the manifest, password, or submission gateway

#### Scenario: Diagnostic validation rejects an operational mistake
- **WHEN** the manifest carries a mismatched manifest version, an unknown phase, an elapsed expiry, an out-of-range tool call limit, a relative or non-`.sock` submission socket path, or a missing/non-canonical runtime config snapshot, or the DB password environment value is empty
- **THEN** bootstrap fails before the MCP server starts, with a message that names the operational condition

#### Scenario: Effective allowlist comes from the catalog, not the manifest
- **WHEN** bootstrap resolves the MCP tool allowlist for a valid phase
- **THEN** the effective allowlist equals `McpToolContractCatalog`'s canonical tool set for that phase, and a manifest whose `allowedTools` payload diverges from the canonical set cannot broaden or narrow the tools the MCP server exposes

#### Scenario: Bootstrap does not re-verify writer-derived identity fields
- **WHEN** bootstrap decodes a manifest produced by the production writer
- **THEN** it does not require tool schema hash equality, canonical allowlist equality, invocation/decision-run identity equality, hash length shapes, or prompt version non-emptiness, because the writer and the submission gateway already enforce those invariants at write and submission time
...
### Requirement: Authentication suspicion follows observed evidence, not the resolved category

Issue #306 observability invariant: The audit signal `authFailureSuspected` MUST be true whenever the invocation's resolved primary failure category is `AUTHENTICATION` **or** authentication evidence was independently observed (`authEvidenceObserved`), so an authentication failure that the parser's first-match resolution assigned to a different category is still recorded and logged as an authentication suspicion.
...
- **WHEN** a Codex invocation's stdout fails the pinned JSONL contract so the resolved primary category is `OUTPUT_CONTRACT`, and its stderr contains `Failed to refresh token`
- **THEN** the phase audit details record `authFailureSuspected` as true and the configured authentication runbook message is emitted to the operational log

#### Scenario: Refresh-token failure withholds raw output
...
- **WHEN** a Codex invocation resolves to a non-`AUTHENTICATION` category and neither stdout nor stderr contains any known authentication-evidence string, and no parsed event message mapped to `AUTHENTICATION`
- **THEN** the phase audit details omit `authFailureSuspected` entirely and no authentication runbook message is emitted

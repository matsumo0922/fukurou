...
- [ ] 1.2 `run_backup()` から watchdog の起動・回収・result file 判定と、`DUMP_DEADLINE_SECONDS` の範囲検証・host 側 `timeout --signal=TERM ...` ラッパーを削除する。パイプラインは `PIPESTATUS` による producer / restic の exit 判定を維持する
- [ ] 1.2b dump producer を container 内側の `timeout --signal=TERM --kill-after=10 840 pg_dump ...` で bound する（`docker exec` の引数として container 内 timeout を使う。host 側 exec client の死と無関係に pg_dump を終了させ、孤児化を防ぐ。postgres:16 は GNU coreutils timeout 9.7 で `--kill-after` 対応を実機確認済み。design.md Decision 1・反証ゲート blocking 反例対応）
- [ ] 1.2c 環境変数 `FUKUROU_BACKUP_DUMP_DEADLINE_SECONDS` は宣言ごと削除する（bound は 840 固定。旧 env を設定していても無視される状態を作らず、参照自体を消す）
...
- [ ] 2.1 `backup-fukurou` の `restic_command()` と partial snapshot cleanup（62 行付近）の `--no-cache` を削除する。`restore-fukurou`（229, 243 行付近）と `restore-selftest`（296 行の `--no-cache` 存在 assertion）は**変更しない**: restore drill は使い捨て環境での読み取りで cache の恩恵がなく背圧問題も存在しないため、撤去の根拠が backup 側と異なる。restore 側の cache 方針は scope 外とする（反証ゲート反例 3 対応）

...
**Non-Goals**（ユーザー確認済み: issue #287 本文「やらないこと」の転記）:
...
- **`mcpServerConfig()` に `forwardedEnvironmentVariables = listOf(FUKUROU_INVOCATION_ID_ENV)` を無条件で設定する**（ユーザー確認済み: issue #287 の「やること」1）。provider 分岐は行わない。理由: `mcpServerConfig()` は provider に関わらず `OneShotLlmRequestFactory.llmRequest()` から共通で呼ばれる shared builder であり（falsify で確認済み: `OneShotLlmRunner.kt` L2037-2059）、`forwardedEnvironmentVariables` フィールド自体は Codex renderer（`renderCodex` → `toCodexConfigToml`）だけが読み、Claude renderer（`renderClaude` → `toClaudeMcpConfigJson`）はこのフィールドを一切参照しないため、provider ガードなしで無条件設定しても Claude 経路の wire contract に影響しない
...
- **テストは falsifier ではなく `proposerProvider = LlmProvider.CODEX` を指定した Codex proposer launch を対象にする**（agent 仮決め、falsify で指摘・採用）。理由: issue #287 の障害は「Codex proposer run の全滅」であり、既定 provider（falsifier=CODEX）を流用するより、障害の主語である proposer を直接固定した回帰テストの方が incident との対応が明確

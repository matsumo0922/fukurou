...
query は監視条件に対応する partial index、固定 row limit、transaction-local statement timeout を利用し、raw payload や row list を application DTO に渡さない。market-data gap は `recovered_at IS NULL`、infrastructure gap は `OPEN` に対応する `CLOSE` が存在しない行だけを unresolved 集合としてから上限判定する。解決済み全履歴の成長を unresolved 上限へ混ぜず、unresolved 集合自体が limit を超えた場合だけ `UNKNOWN` にする。query ごとの例外/timeout は component-local `UNKNOWN` へ変換し、他 component を巻き込まない。件数には明示した上限を設け、上限到達時は切り捨てず `UNKNOWN` とする。

...
- [ ] 1.3 database引数を受けないproduction-safe factoryにsingle owned DataSourceから`ExposedDatabase`、scope connection factory / evictorだけを生成するearly opaque `PostgresStorageRoot`を追加する
- [ ] 1.4 active DB runtime config / clock解決後のroot overloadでledger / decision / policy repositoryとbackendをconfig-bound componentとして同時生成し、same root rebuildはnew configでbound componentだけを再生成する
...
- [ ] 1.6 daemon schedulerとmanual launchのruntime input / `createLlmLaunchRuntimeComponents`をpairから同じApplication root + resolved config / clock overloadへ移す
- [ ] 1.7 shared Application runtime closeはbound resourceだけを閉じ、workers終了後のApplication shutdownがrootを一度だけcloseするownershipを実装する
...
- [ ] 1.12 InMemory scopeをcancellable identity-keyed coroutine mutexで実装し、holder / waiter参照がなくなったregistry entryだけを安全に除去する
...
- [ ] 1.14 Hikari borrow前に`System.nanoTime`の30秒deadlineを開始し、deadline-aware root connection factoryと50ms cancellation-aware delayで`pg_try_advisory_lock`をpollするcode-owned定数 / KDocを追加する
...
- [ ] 1.19 lock順をstable request two-int advisory -> market session single-bigint advisory -> market session row -> risk state -> account -> positions -> ordersに固定し、external I/O中はstable scope以外のlockを保持しない
...
- [ ] 4.3 same root / new active config rebuildがnew bound componentを作りrootを維持し、shared runtime close後のApplication shutdownとstandalone runtime closeがrootを各ownershipで一度だけcloseすることをtestする
...
- [ ] 4.5 PostgreSQL別connection / 別backend instanceのsame-request barrierでlive session存続中のcross-process直列化をtestし、異なるstable requestのhash32 collisionはover-serializeだけでauthority / resultを共有しないことをtestする
...
- [ ] 4.7 module-internal短縮deadline seamでHikari borrow stallとholder継続中のtry-lock false pollがmonotonic timeoutし、definite false + timeout restore成功時だけconnectionを通常pool returnできることをtestする
...
- [ ] 4.10 eligibility付きresting scopeとmarket eventをbarrierで交差させ、stable scope以外をexternal I/O中に保持せず所定lock順でdeadlockしないことをtestする
...
- [ ] 5.1 `docs/mcp-runtime.md`をA2b internal path接続済み、live session中のstable scope / 独立5秒heartbeatで保護され、confirmed tradeのpaper truth / strategy evaluationを維持し、losing unavailable attempt / duplicate preparation / auditだけをinfra residualとして報告し、Bまでpublic/MCP/runnerから到達不能である現在形へ更新する

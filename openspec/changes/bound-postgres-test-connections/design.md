...
各 Gradle module の test source に `PostgreSQLContainer` extension を置き、fixture construction 時に `withUrlParam` で既定の `connectTimeout=10`、`socketTimeout=30` を設定する。container が生成する URL 自体へ設定するため、HikariCP、Exposed、`DriverManager`、runtime config の全 consumer に同じ上限が伝播する。
...
### [agent 仮決め] 既定 timeout 値は connect 10 秒、socket read 30 秒とする

localhost の Testcontainers 接続として connect 10 秒は十分な余裕を持ち、authentication を含む socket inactivity は 30 秒で打ち切る。値は module 内の test helper が所有し、contract test が既定 URL parameter と「consumer 固有の値はこの上限以下」を固定する。30 秒の妥当性は targeted/full test が既存の正当な PostgreSQL 操作を完走することで観測する。

### [agent 仮決め] `runPostgresTest` の初期接続だけを最大 2 attempts にする

初回 full validation で timeout 自体は約 30 秒で発動したが、`PostgresPersistenceIntegrationTest` の pool 初期化がそのまま失敗し、full validation の完走条件を満たさなかった。そこで `runPostgresTest` が test body を開始する前の DataSource construction に限り、`SocketTimeoutException` または `ConnectException` を cause chain に持つ失敗を 1 回だけ再試行する。2 回目の失敗、authentication failure、SQL error、test body 内の失敗はそのまま伝播する。
...
- [retry が systematic failure を隠す] → retryable cause を socket timeout / connect failure に限定し、最大 2 attempts、test body 開始前だけに適用する。
- [新しい PostgreSQL fixture が helper 適用を忘れる] → module ごとの contract test と review 時の `PostgreSQLContainer` finite inventory（9 files / 10 construction sites）で検出する。

...
### D1: 正常集計と oversized rejection の population を分離する（agent 仮決め）

正常経路は current scope の最小行だけで attribution と prior PnL を確認する。oversized rejection は entity limit を超える最小の単一 population を直接作り、full report 集計や三表一括更新を前提にしない。これにより oracle は transport timeout ではなく typed `ENTITY_LIMIT` のままになる。
...
- [Risk] population fixture の縮小で境界前の正常集計 coverage を失う → 正常 scoped 集計と oversized typed rejection を独立 assertion として維持する。

...
- Falsifier が承認前に `preview_order` を呼べない canonical policy にする。
...
- provider、model、prompt 本文、注文 lifecycle、DB schema を変更しない。

## Decisions

1. **（ユーザー確認済み）`preview_order` を Falsifier phase の canonical allowlist から除外する。**
...
2. **（agent 仮決め）`preview_order` の schema registration は phase allowlist と分離して維持する。**
   ...
   - catalog の `allTools` には preview schema を明示的に残し、tool の wire schema drift を起こさない。

3. **（agent 仮決め）回帰証明は production request manifest と runner ordering の 2 点で行う。**
   - Falsifier manifest に `preview_order` が存在しないことを、実際の `OneShotLlmRunner` request construction 経由で固定する。
...
- [phase allowlist から外した tool の schema registration が誤って消える] → `allTools` に preview schema を明示し、MCP tool contract test を維持する。
...
2. deploy 後の新規 Falsifier process から `preview_order` が消える。DB migration と runtime config activation は不要。

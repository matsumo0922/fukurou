...
- [ ] 1.2 Add required phase-owned `ToolPolicy` with no default, migrate pre-filter, proposer, falsifier, Reflection, report, manual, and test callers, and admit only the canonical RISK_REDUCTION_ONLY policy through the fixed supervisor
...
- [ ] 1.4 Add pinned-version Claude result and Codex JSONL adapters that separate semantic response, usage, effective-name-or-CLI-default configured identity, provider-observed identity, safe provider detail, and schema validation
...
- [ ] 1.7 Add rendering, auth absence, empty/missing/exact risk-reduction tool policy, schema drift, entry rejection with safety-reduction preservation, compound failure precedence, model attribution, barrier, adapter-pin agreement, and no-session-scan tests

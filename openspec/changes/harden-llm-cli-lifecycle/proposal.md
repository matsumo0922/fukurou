...
- Claude no-MCP 呼び出しから認証を無効化する `--bare` を廃止し、strict empty MCP config、copied auth、明示 model、空 tool policy を同時に成立させる。
...
- **この stage（PR 1）**: release barrier、no-MCP auth/tool policy、typed provider output/failure、model attribution。
...
- Docker image の Claude/Codex pin、runtime supervisor/launcher、production credential、実資金取引、Issue #154 activation は変更しない。

...
- [x] 3.1 `COMMITTED` を後続要求で上書きしない state 更新規則を実装する
- [x] 3.2 「commit 後の conflict で `COMMITTED` を維持する」テストを追加する
- [x] 3.3 「拒否のあとの受理で `COMMITTED` になる」テストを追加する
...
- [x] 5.1 同一接続での「拒否 → 受理」が成立するテストを追加する
- [x] 5.2 接続を張り直した「拒否 → 受理」が成立するテストを追加する
- [x] 5.3 要求を送らずに切断しても gateway が後続接続を受け付けるテストを追加する

...
| sandbox/tool inventory | canonical `ToolPolicy` とadapter | launcher、MCP/DB/secret/process isolation | app UID、read-only rootfs、data-free fixture、empty policy、Codex `submit_falsification` auto-approval実call | 同一digest qualificationが必要 | final digest証跡待ち |

...
- [ ] 1.1 Add failing workflow contract tests for event-derived intent, historical quality enforcement, reason/mode validation, schema-sensitive early blocking, bundle v2 fields, and installed contract v2.
- [ ] 1.2 Add failing deploy contract/runtime fixtures for bundle tamper rejection, inventory hash agreement, lock-time ancestry, fresh install, mode mismatch, evidence persistence, and roll-forward-only recovery.

## 2. Workflow and signed bundle v2

- [ ] 2.1 Add manual migration mode and reason inputs, derive `FORWARD` or `AUTHORIZED_ROLLBACK` from event/target, and require quality for every target.
- [ ] 2.2 Add the versioned schema-sensitive path inventory and block known schema-sensitive automatic pushes before image publication.
- [ ] 2.3 Extend canonical signed bundle generation and JSON schema with closed v2 event, intent, reason, migration mode, and inventory hash fields.

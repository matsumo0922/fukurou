...
v2 executorをv1へ戻す操作はunfinished v2 stateが1件でもある間は禁止する。v1 binaryはv2の`ROLL_FORWARD_ONLY`を理解できず機構的なdowngrade互換を後付けできないため、v2 stateをv2 executorでterminalへ収束させたreadbackをroot downgradeの前提にする。
...
5. rollbackする場合は新 workflowを止め、v2 executorでv1/v2 journal recoveryをterminalへ完了する。unfinished v2 stateが0であるreadback前にroot executorをv1へdowngradeしない。DB historyは戻さない。

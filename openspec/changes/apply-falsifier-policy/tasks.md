...
- [ ] 3.4 policy decision の machine-readable pre-mutation audit payload と authority に合う注文理由を追加する
...
- [ ] 4.4 commit 済み client request は policy read を再要求せず既存 result を mutation なしで replay する
...
- [ ] 5.3 broker/SafetyFloor の正規 permit、MCP permit 欠損、tamper、ADD_LONG、read failure の回帰テストを追加する
- [ ] 5.4 commit 後 ACK loss と policy repository outage の idempotent replay 回帰テストを追加する

...
cleanup attemptはまずatomic readbackを行い、open position 0かつrisk-increasing order 0ならmarket tickなしでもSAFEを保存する。open positionがありsource timestamp が不正、5秒を超えてstale、または現在より5秒を超えて未来の REST ticker しか取得できない場合はmutationせずUNKNOWNを維持する。order cancelだけなら価格入力を要求しない。

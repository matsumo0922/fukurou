...
（agent 仮決め）`PaperLedgerMutationRepository` に atomic risk-exit requestを追加する。requestは実行理由、audit context、close対象ごとの事前計算済みmarket fill、およびscopeを持つ。
...
- `ALL_OPEN_RISK`: HARD_HALT用。全risk-increasing open orderを取消し、requestに含まれる全open positionをcloseする。
...
（agent 仮決め）`risk_state`にadditiveなcleanup stateを追加し、中間phaseを増やさず`UNKNOWN`と`SAFE`だけを表す。新規HARD_HALT transitionはUNKNOWNを保存する。すでにHARD_HALT/SAFEなら重複setでUNKNOWNへ戻さない。atomic cleanup transactionは全mutation後にopen position 0かつrisk-increasing order 0を同じlock内で再確認できた場合だけSAFEを保存する。

transaction commit後に応答を失っても、次passはSAFEとempty target setをidempotent successとして扱いduplicate executionを作らない。DB結果を確定できないfailureではSAFEへ昇格せず、既存reconciler failure auditとsticky HARD_HALT/UNKNOWNを維持する。manual resumeはSAFE以外で拒否し、cleanup未完了の迂回を防ぐ。resume成功時はcleanup evidenceを非HARD_HALT状態へ戻す。

### 6. retryは既存startup/periodic reconcilerへ限定する

（agent 仮決め）production `ProtectionReconcilerWorker` のbootstrap完了後かつWebSocket接続前に、既存のstartup reconcile entrypointでHARD_HALT/UNKNOWN cleanupを一度試みる。通常loopとperiodic safety maintenanceは、各passでsticky HARD_HALTとcleanup stateを読み、SAFEになるまで同じcleanupを再試行する。kill criterionとdrawdown transitionも同じbroker operationへ接続する。

startup時にtickを取得できなければmutationせず、periodic/event passへ持ち越す。新worker、新scheduler、新leaseは作らない。
...
6. application bootstrapによるadditive schema upgradeで通常deployする。history backfill、destructive migration、NAS bootstrapは行わない。旧readerは追加columnを無視でき、rollback時もcolumn/値を削除しない。

...
- `safety_floor_margin_reports` — `id` / `evaluation_path` / **`call_site`**（PREVIEW / PLACE）/ `decision_run_id` / `command_id` / `order_id` / `verdict` / `rejected_rule`(nullable) / `policy_version` / `runtime_config_version` / **`observation_schema_version`** / `divergence` / `observed_at`

...
- Remove wall-clock `market_eligible_from` from the active entry-fill predicate while retaining additive rollback compatibility.
...
- PostgreSQL schema/bootstrap: one nullable additive order boundary column and receipt lookup/capture queries; no destructive migration or history rewrite.
- Tests: WebSocket receipt propagation, deterministic PostgreSQL commit-order races, missing/legacy linkage, schema upgrade/old-reader compatibility, and production `ProtectionReconcilerWorker` wiring.

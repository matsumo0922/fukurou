...
- [systemd `ExecStopPost` 自体が実行不能になる] → stale `RUNNING` と projection age を endpoint に出し、PR-5 が failure と評価できる。publisher は unit sandbox の write allowlist 内だけを更新する。

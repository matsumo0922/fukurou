...
- （agent 仮決め）production で requested/executed audit が一度も作成されていない既知状態に基づき、注入専用 `CommandEventType` と activity catalog 表示も削除する。DB row の削除や backfill は行わない。
...
- `trading` の injected disconnect interface、GMO WebSocket session mutation、command-event primary-key reader と専用 tests。

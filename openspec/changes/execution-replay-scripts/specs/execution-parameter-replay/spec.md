...
- 約定レイテンシが記録済み execution の処理時刻から一意に決まる
- 短縮 TTL の retention が、候補の論理期限と記録済み execution の処理時刻の比較から一意に決まる
...
- **THEN** レイテンシは記録済み execution の処理時刻と発注時刻の差であり、その行は `fidelity` が `EXACT` である

#### Scenario: Retention is judged against the recorded processing time

- **WHEN** 短縮 TTL の論理期限が記録済み execution の処理時刻より後にある
- **THEN** その候補は約定を保つ (RETAINED) と `EXACT` で判定される

### Requirement: TTL counterfactual is shortening-only and anchored to recorded fills

system SHALL TTL 反実仮想の対象を、記録済み TTL 以下への短縮に限る。system SHALL NOT TTL を延長した場合の約定を出力する。

system SHALL 約定の権威を記録済み execution 行に置く。約定した order の retention 判定は、候補 TTL の論理期限 `created_at + T'` を記録済み execution の処理時刻 `executed_at` と比較して行う。`E'` が `executed_at` 以下なら取りこぼす (DROPPED)、後なら保つ (RETAINED)。`E'` が `executed_at` とミリ秒単位で一致する退化ケースは `PROCESSING_CLOCK_TIE` で `UNKNOWN` とする。
...
- **WHEN** 記録済みでは約定した order に、その execution の処理時刻より前で失効する短縮 TTL を与える
- **THEN** その候補は約定を取りこぼす (DROPPED) と `EXACT` で判定される
...
#### Scenario: Expiry ties the fill processing time

- **WHEN** 候補の論理期限が execution の処理時刻とミリ秒単位で一致する
- **THEN** その候補は `PROCESSING_CLOCK_TIE` で `UNKNOWN` となる

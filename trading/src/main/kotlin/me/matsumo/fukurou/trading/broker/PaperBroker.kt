    ...
    override suspend fun getAccountStatus(): Result<AccountStatus> {
        return runCatching {
            ...
            AccountStatus(
                ...
                protectionStatus = protectionStatus(positions, openOrders),
            )
        }
    }

    private fun protectionStatus(positions: List<Position>, openOrders: List<Order>): ProtectionStatus {
        val openPositions = positions.filter { position -> position.status == PositionStatus.OPEN }
        val protectedPositionCount = openPositions.count { position -> position.currentStopLossJpy != null }

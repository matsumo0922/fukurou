...
private fun Route.registerOpsLlmAuthStatusRoute(llmAuthService: LlmAuthService?) {
    get("/ops/llm-auth") {
        ...
        description = "Claude Code / Codex CLI の login state を専用 endpoint で返します。" +
            "token や credential file の内容は返しません。" +
            "status は credential marker の存在だけでなく、現在の credential を使った LLM 起動が" +
            "認証失敗を観測したかを反映します。" +
            "logged_in は marker が存在し認証失敗を観測していない状態、" +
...
private fun CommandEventType.toActivityAuditEventDefinition(): OpsActivityCatalogItemResponse {
    val keySuffix = when (this) {
        ...
        CommandEventType.LLM_EXECUTION_ADMISSION_BLOCKER_RESOLVED -> "llmExecutionAdmissionBlockerResolved"

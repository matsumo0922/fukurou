...
[確定] 本システムは、Docker(Linux)上で24時間常駐するKotlin/JVM製の暗号資産デイトレAI botである。対象はBTC一本、現物、GMOコイン取引所側に限定し、当初は仮想10万円のペーパートレードから開始する。合格ラインは3ヶ月ペーパーで `PF > 1.2` かつ `最大DD < 15%` とし、達成後に少額実弾へ移行する。
...
[確定] デーモンが唯一のマクロ・スケジューラであり、発火ごとにCLIをワンショット起動する。LLMはCLIシェルアウトで実行し、`claude` / `codex` を `LlmInvoker` 抽象で差し替える。構造化は自由文パースではなく、MCPツール呼び出し、特に `decision.submit_decision` によって担保する。

[確定] ログと知識層は、SQLite(Exposed)を機械的な真実、Obsidian Vaultを人間が読む知識として二本立てにする。振り返りエージェントが日次・週次で `Knowledge/` を書き、Dataviewで成績・override・失敗パターンを可視化する。
...
| 運用ルール層 | 確信度閾値、発火閾値、ATR係数、クールダウン等 | 可能、理由必須 | daemon / MCP / config |
| 安全床層 | 2%損失、SL必須、ナンピン禁止、DD停止、エクスポージャー、残高・レート上限 | 不可 | `gmo-coin-mcp` server-side |
...
- ペーパー約定は単なる終値約定ではなく、板、スプレッド、手数料、スリッページ、部分約定を近似し、本番乖離を減らす。
...
[仮定] GMOコインAPIの銘柄表記はエンドポイントや商品区分で差がありうるため、実装では `TradingSymbol.BTC_JPY` を内部表現、GMO APIへ渡す値を `GmoSymbolMapper` で変換する。起動時に `/public/v1/symbols` の取引ルールを取得し、現物で利用可能なsymbol、最小数量、刻み、maker/taker feeを検証する。

[確定] GMOコイン現物APIは `executionType: STOP`（逆指値）および `OCO` に対応している（確認済み）。したがってv1は「取引所ネイティブの逆指値注文を第一の保護手段」とし、bot/デーモンが停止していても取引所側で建玉が守られる状態を基本とする。デーモン/MCPが監視するサーバーサイド保護ストップは、ネイティブSTOPが置けない/失効した場合のバックアップとして併設する。少額実弾移行前に、ネイティブSTOPのトリガ挙動（成行執行・スリッページ・資金拘束・部分約定）を実機で検証する。
...
| コンポーネント | 実装モジュール | 責務 |
|---|---|---|
| Kotlin Daemon | `bot-daemon` | 常駐、定期発火、イベント発火、CLI起動、タイムアウト、キルスイッチ、復旧、通知 |
| Trigger Engine | `bot-daemon` / `market-*` | 15分定期、±1%/5分、保有中の密発火、クールダウン制御 |
| LLM CLI Runner | `llm-cli` | `claude` / `codex` をワンショット起動し、runId・MCP設定・初期プロンプトを渡す |
| MCP Server | `gmo-coin-mcp` | GMO Public/Private REST+WS、read系/act系ツール、risk preview、安全床強制、決定記録 |
| Safety Floor | `execution-core` + `gmo-coin-mcp` | 発注・ストップ調整・買い増し・overrideをサーバー側で検証し拒否ログを残す |
| Paper Simulator | `paper-simulator` | 手数料、スプレッド、スリッページ、板歩き、部分約定、保護ストップを近似 |
| Persistence | `persistence-exposed` | SQLite/将来PostgresのRepository実装、WAL、集計テーブル |
| Obsidian Writer | `knowledge-obsidian` | frontmatter付きMarkdown生成、Dataview互換、MOC更新 |
| Reflection Agent | `reflection-agent` | 日次・週次の振り返り、Knowledge更新、確信度較正、失敗パターン抽出 |
| Ktor WebUI | `web-ktor` | 後日: 状態確認、手動停止、成績、overrideレビュー、設定閲覧 |

### 2.3 データフロー

#### 2.3.1 市場データの流れ

1. `gmo-coin-mcp` がGMO Public WebSocketから ticker/orderbook/trades を購読する。
2. RESTで5分・1時間・日足のOHLCVを補完する。
3. `MarketDataSource` 実装が、ローソク足、板、約定、指標、マイクロストラクチャ要約を統一モデルへ変換する。
4. 必要なスナップショットのみSQLiteへ保存する。高頻度の板フル履歴はv1では保存せず、要約特徴量を保存する。
5. LLMは初期プロンプトで全データを受け取らず、MCP read系ツールで必要な粒度を取得する。
...
sequenceDiagram
    ...
    MCP->>GMO: Public/Private REST/WS取得
    MCP->>DB: ToolCall保存
    CLI->>MCP: risk.preview_order
    MCP->>MCP: Safety Floor検証
    CLI->>MCP: decision.submit_decision
    MCP->>DB: Decision保存
    CLI->>MCP: trade.place_order / adjust_stop / close_position
...
├── modules/
│   ├── common-config/
│   │   └── src/main/kotlin/me/matsumo/fukurou/config/
│   ├── common-logging/
│   │   └── src/main/kotlin/me/matsumo/fukurou/logging/
│   ├── bot-domain/
│   │   └── src/main/kotlin/me/matsumo/fukurou/domain/
│   ├── bot-daemon/
│   │   └── src/main/kotlin/me/matsumo/fukurou/daemon/
│   ├── llm-cli/
│   │   └── src/main/kotlin/me/matsumo/fukurou/llm/
│   ├── gmo-client/
│   │   └── src/main/kotlin/me/matsumo/fukurou/gmo/
│   ├── gmo-coin-mcp/
│   │   └── src/main/kotlin/me/matsumo/fukurou/mcp/
│   ├── market-core/
│   │   └── src/main/kotlin/me/matsumo/fukurou/market/
│   ├── execution-core/
│   │   └── src/main/kotlin/me/matsumo/fukurou/execution/
│   ├── paper-simulator/
│   │   └── src/main/kotlin/me/matsumo/fukurou/paper/
│   ├── persistence-exposed/
│   │   └── src/main/kotlin/me/matsumo/fukurou/persistence/
│   ├── knowledge-obsidian/
│   │   └── src/main/kotlin/me/matsumo/fukurou/knowledge/
│   ├── reflection-agent/
│   │   └── src/main/kotlin/me/matsumo/fukurou/reflection/
│   ├── web-ktor/
│   │   └── src/main/kotlin/me/matsumo/fukurou/web/
│   └── test-fixtures/
│       └── src/main/kotlin/me/matsumo/fukurou/fixtures/
...
```text
bot-domain           依存なし。純粋な型とinterfaceのみ。
common-config        bot-domainに依存可。環境変数と設定ロード。
market-core          bot-domainに依存。指標計算、データ正規化。
gmo-client           bot-domain, common-configに依存。GMO APIクライアント。
execution-core       bot-domainに依存。SafetyFloor、Risk、Sizing。
paper-simulator      execution-core, market-coreに依存。
persistence-exposed  bot-domainに依存。Repository実装。
knowledge-obsidian   bot-domainに依存。Markdown生成。
llm-cli              bot-domain, common-configに依存。CLI起動のみ。
gmo-coin-mcp         gmo-client, execution-core, paper-simulator, persistence, knowledgeに依存。
bot-daemon           llm-cli, persistence, market-core, execution-coreに依存。
reflection-agent     llm-cli, persistence, knowledgeに依存。act系取引ツールには依存しない。
web-ktor             persistence, knowledgeに依存。後日拡張。
```

### 3.3 パッケージ命名

```text
me.matsumo.fukurou.domain.model
me.matsumo.fukurou.domain.port
me.matsumo.fukurou.domain.service
me.matsumo.fukurou.market.indicator
me.matsumo.fukurou.market.trigger
me.matsumo.fukurou.execution.risk
me.matsumo.fukurou.execution.order
me.matsumo.fukurou.paper
me.matsumo.fukurou.gmo.rest
me.matsumo.fukurou.gmo.websocket
...
me.matsumo.fukurou.persistence.table
me.matsumo.fukurou.persistence.repository
me.matsumo.fukurou.knowledge.obsidian
me.matsumo.fukurou.llm.cli
me.matsumo.fukurou.daemon.scheduler
me.matsumo.fukurou.daemon.recovery
me.matsumo.fukurou.reflection
...
- フェーズ1（ペーパーで1周回す最小構成）: `bot-domain` / `common-config` / `market-core` / `gmo-client` / `execution-core` / `paper-simulator` / `persistence-exposed` / `gmo-coin-mcp`（最小ツール: `market.*` / `account.*` / `risk.*` / `decision.submit_decision` / `trade.*`）/ `llm-cli` / `bot-daemon`
- フェーズ2（学習の核）: `knowledge-obsidian` / `reflection-agent` / Dataview
- フェーズ3（拡張）: `web-ktor` / 追加データ源(B-1) / 通知(`Notifier`)
...
```kotlin
package me.matsumo.fukurou.domain.model
...
/**
 * 注文種別。
 * STOPは保護ストップの論理表現であり、実取引所の対応可否はExecutionGatewayが吸収する。
 */
...
```kotlin
package me.matsumo.fukurou.domain.port

import me.matsumo.fukurou.domain.model.Candle
import me.matsumo.fukurou.domain.model.EquitySnapshot
import me.matsumo.fukurou.domain.model.FillExecution
import me.matsumo.fukurou.domain.model.IndicatorSnapshot
import me.matsumo.fukurou.domain.model.LlmDecision
import me.matsumo.fukurou.domain.model.MicrostructureSnapshot
import me.matsumo.fukurou.domain.model.OrderBookSnapshot
import me.matsumo.fukurou.domain.model.OrderRequest
import me.matsumo.fukurou.domain.model.OrderResult
import me.matsumo.fukurou.domain.model.PositionSnapshot
import me.matsumo.fukurou.domain.model.SafetyContext
import me.matsumo.fukurou.domain.model.SafetyVerdict
import me.matsumo.fukurou.domain.model.TickerSnapshot
import me.matsumo.fukurou.domain.model.Timeframe
import me.matsumo.fukurou.domain.model.TradePrint
import me.matsumo.fukurou.domain.model.TradeRule
import me.matsumo.fukurou.domain.model.TradingSymbol
import java.time.Instant
import java.util.UUID
import kotlinx.collections.immutable.ImmutableList
...
interface ExecutionGateway {
    ...
    suspend fun adjustStop(
        command: AdjustStopCommand,
        safetyContext: SafetyContext,
    ): Result<OrderResult>
}
...
import me.matsumo.fukurou.domain.model.TriggerKind
import me.matsumo.fukurou.domain.model.TradingSymbol
import java.time.Duration
import java.time.Instant
import java.util.UUID
...
[設計提案] domain型をそのままUIへ露出せず、`web-ktor` または将来のCompose UIでViewStateへ変換する。
...
|---|---|---|---|---|
| Ticker | GMO Public REST/WS | `MarketDataSource.getTicker` | 現在価格、bid/ask、スプレッド | 最新値＋鮮度 |
| KLine/OHLCV | GMO Public REST | `getCandles` | MTF環境認識、ATR、移動平均 | 指標要約＋必要時ローソク |
| OrderBook | GMO Public REST/WS | `getOrderBook` | スプレッド、板厚、需給偏り | top N＋要約特徴量 |
| Trades | GMO Public REST/WS | `getRecentTrades` | 歩み値、出来高デルタ、スイープ検出 | 直近要約＋異常フラグ |
...
| Open Orders / Positions | GMO Private REST / Paper ledger | account tool | 建玉、保護ストップ、注文整合性 | 判断履歴・安全床 |

### 5.2 GMO APIマッピング方針

[設計提案] GMO APIの具体エンドポイントは `gmo-client` に閉じ込める。MCPツール・domain層は `TradingSymbol` と統一モデルのみを扱う。

| domain操作 | GMO API候補 | 備考 |
|---|---|---|
| ticker | `GET /public/v1/ticker`、Public WS `ticker` | symbol変換を通す |
| orderbook | `GET /public/v1/orderbooks`、Public WS `orderbooks` | WSはsnapshotとして扱い、鮮度を記録 |
| trades | `GET /public/v1/trades`、Public WS `trades` | 直近約定と出来高デルタに使用 |
| candles | `GET /public/v1/klines` | 5min/1hour/1dayを取得 |
...
| place order | `POST /private/v1/order` | live移行後のみ。必ずSafety Floor通過後 |
| change order | `POST /private/v1/changeOrder` | ストップ/指値変更に利用可能なら使用 |
| cancel order | `POST /private/v1/cancelOrder` | 復旧・整理に使用 |

[確定] GMO現物は `executionType: STOP`/`OCO` に対応。`ExecutionGateway` 内の `StopPlacementStrategy` は `NATIVE`（取引所に逆指値を常駐させ、bot停止中も作動）を既定とし、監視型の `SYNTHETIC`（daemon/MCPが到達検知して決済）はフォールバックとする。実弾前にネイティブSTOPの実挙動を検証する。
...
| P2 | ETF/マクロカレンダー | 先行イベント | カレンダーAPI | `TriggerSource` | 重要イベント前のサイズ抑制 | 中 | BTC短期ボラの回避目的 |
...
[確定] 発火はハイブリッド。定期15分、イベント `±1%/5分`、保有中は密、クールダウン既定5分。

[設計提案] v1の具体条件は次の通り。

| 発火 | 条件 | cooldown | 備考 |
|---|---|---:|---|
| 定期 | 15分ごと、JST基準で `00,15,30,45` | 適用 | 基本判断 |
...
```kotlin
package me.matsumo.fukurou.llm.cli

import me.matsumo.fukurou.domain.port.LlmInvoker
import me.matsumo.fukurou.domain.port.LlmRunRequest
import me.matsumo.fukurou.domain.port.LlmRunResult
...
- confidence < 0.6 の場合は原則 NO_TRADE。override希望は ops.request_override で理由を出す。
...
5. 発注が必要な場合のみ trade.place_order / close_position / adjust_stop を呼ぶ。
...
{
  "runId": "uuid",
  "action": "ENTER_LONG | ADD_LONG | HOLD | REDUCE | CLOSE | ADJUST_STOP | NO_TRADE",
  ...
  "entryPlan": {
    "preferredOrderType": "MARKET | LIMIT | STOP | STOP_LIMIT",
...
| 過信 | confidence cap、過去の確信度別成績の週次較正 |
...
| action | 必須tool |
|---|---|
| `ENTER_LONG` | ticker, candles(5m/1h/1d), orderbook, recent_trades, trade_rules, balances, positions, risk.preview_order, knowledge.recent_lessons |
| `ADD_LONG` | 上記 + current_position + pyramid_status |
| `ADJUST_STOP` | positions, indicator ATR, risk.preview_stop_adjustment |
...
[設計提案] LLMの自己申告confidenceに加え、MCP側で「最大許容confidence」を算出して保存する。`decision.submit_decision` はconfidenceそのものを変更しないが、`calibratedConfidence = min(llmConfidence, cap)` を別フィールドに保存し、発注時の閾値判定にはcalibratedを使う。
...
```text
canAct = calibratedConfidence >= effectiveConfidenceThreshold
      && decision.action in actionableActions
...
[確定] `gmo-coin-mcp` はGMOコイン暗号資産のPublic/Private REST＋WebSocketをラップする自作MCPサーバーであり、MCP公式Kotlin SDKで実装する。
...
account.get_equity_summary

risk.get_safety_status
risk.preview_order
risk.preview_stop_adjustment
...
```json
{
  ...
  "source": "GMO_PUBLIC_WS | GMO_PUBLIC_REST | PAPER_LEDGER | SQLITE | OBSIDIAN",
  "stalenessMs": 1200,
  "data": {},
  "warningsJa": []
}
...
```json
{
  ...
  "required": ["symbol"],
  "properties": {
    "symbol": { "type": "string", "enum": ["BTC_JPY"] }
  }
}
```
...
```json
{
  ...
  "mtf": {
    "1d": { "bias": "UP", "atr14Jpy": "220000", "notesJa": "日足は上昇基調。前日高値が近い。" },
    "1h": { "bias": "UP", "atr14Jpy": "52000", "notesJa": "EMA20上で推移。" },
    "5m": { "bias": "PULLBACK", "atr14Jpy": "8500", "notesJa": "急伸後の押し。出来高増。" }
  },
  "microstructure": {
    "spreadBps": "1.0",
    "imbalanceTop5": "0.18",
...
- `confidence` は0〜1。
- actionable actionで `confidence < effectiveThreshold` の場合、`acceptedForAction = false` と保存する。
...
```json
{
  "decisionId": "uuid",
  "accepted": true,
  "acceptedForAction": true,
  "calibratedConfidence": "0.62",
  "effectiveConfidenceThreshold": "0.60",
  "warningsJa": []
}
```
...
{
  "type": "object",
  "required": ["runId", "decisionId", "symbol", "action", "side", "orderType", "sizeBtc", "stopLossJpy", "reasonJa"],
  "properties": {
...
2. `decision.acceptedForAction == true` 確認。
3. account/position/tradeRule/ticker/orderbookを最新化。
4. `SafetyFloor.evaluateOrder` 実行。
5. 拒否なら `safety_violations` と `tool_calls` へ記録して終了。
6. PAPERなら `PaperExecutionGateway.placeOrder`。
7. LIVEなら `GmoExecutionGateway.placeOrder`。
8. order/fill/position/equityを保存。
9. Obsidian trade noteへ追記。
...
{
  "type": "object",
  "required": ["runId", "decisionId", "positionId", "newStopLossJpy", "reasonJa"],
  "properties": {
    "runId": { "type": "string", "format": "uuid" },
    "decisionId": { "type": "string", "format": "uuid" },
    "positionId": { "type": "string", "format": "uuid" },
    "newStopLossJpy": { "type": "string" },
    "reasonJa": { "type": "string", "minLength": 20 }
  }
}
```
...
- `newStopLossJpy < currentStopLossJpy` は緩めとして拒否。
- `newStopLossJpy < atrTrailingFloorJpy` はトレーリング床違反として拒否。
- positionが存在しない場合は拒否。
...
- 既定では、`CONFIDENCE_THRESHOLD` の一時的引き下げは最大 `0.55` まで、かつ1 runのみ。
...
- `trade.adjust_stop` は `newStopLoss >= currentStopLoss` かつ `newStopLoss >= atrTrailingFloor` の場合のみ許可。
- LLMの早期利確は許可するが、ATR床より下へSTOPを緩めることは不可。
- STOP保護は取引所ネイティブ逆指値（`StopPlacementStrategy.NATIVE`）を第一とし、bot停止中も取引所側で作動させる。ネイティブが置けない場合のみ daemon/MCP がSTOP到達を検知して決済する。
...
[確定] 合計エクスポージャー上限を安全床に含める。

[設計提案] 現物・レバなしでも、暴走買いを防ぐため既定は以下。

```yaml
risk:
  maxTotalExposureRatio: 1.0 # 総評価額の100%まで。現物なので1倍を超えない
  maxSingleSymbolExposureRatio: 1.0
  reserveCashJpy: 1000
```

少額実弾移行時は `maxTotalExposureRatio = 0.3` などから開始することを推奨するが、これは確定事項ではなく運用提案。
...
[確定] SQLite(Exposed)が真実/機械・集計、Obsidianが人間の知識。
...
- 将来Postgres移行に備え、Repository interfaceで閉じる。

### 11.2 Exposedテーブル定義レベル

```kotlin
package me.matsumo.fukurou.persistence.table
...
object DecisionsTable : UUIDTable("decisions") {
    ...
    val confidenceThreshold = decimal("confidence_threshold", 5, 4)
...
FROM "Trades"
WHERE type = "trade" AND symbol = "BTC_JPY"
...
const trades = dv.pages('"Trades"')
  .where(page => page.type === 'trade' && page.symbol === 'BTC_JPY' && page.status === 'CLOSED');
...
sqlite:
  jdbcUrl: "jdbc:sqlite:/data/fukurou.sqlite"
  walEnabled: true
  busyTimeoutMs: 5000
...
|---|---|---:|---|
| `GMO_API_KEY` | `gmo-coin-mcp` | いいえ | private REST用 |
| `GMO_SECRET_KEY` | `gmo-coin-mcp` | いいえ | HMAC署名用 |
| `MCP_BEARER_TOKEN` | daemon + MCP | run tokenのみ | LLMには短命token |
| Claude/Codex認証 | daemon containerの専用volume | はい、CLI実行に必要 | exchange secretとは隔離 |
| DB path | daemon/MCP | いいえ | secretではないが権限注意 |
...
- `claude` / `codex` のログインは、専用ユーザー `bot` のDocker volumeへ事前に手動で行う。
- volume例: `codex-auth:/home/bot/.codex`、`claude-auth:/home/bot/.claude`。
- token refreshが必要なCLIもあるため、認証volumeは原則read-write。ただしファイル権限は `0700`。
- daemon containerはexchange secretを持たない。
- `gmo-coin-mcp` はLLM CLI認証を持たない。
...
services:
  bot-daemon:
    ...
    environment:
      APP_MODE: PAPER
      SQLITE_JDBC_URL: jdbc:sqlite:/data/fukurou.sqlite
      OBSIDIAN_VAULT_PATH: /vault
      MCP_SERVER_URL: http://gmo-coin-mcp:8080/mcp
    volumes:
      - bot-data:/data
      - obsidian-vault:/vault
      - codex-auth:/home/bot/.codex
      - claude-auth:/home/bot/.claude
  ...
  web-ktor:
    build:
      context: ..
      dockerfile: docker/Dockerfile.daemon
    command: ["java", "-jar", "/app/web-ktor.jar"]
    profiles: ["web"]
    restart: unless-stopped
    volumes:
      - bot-data:/data:ro
...
| DB busy | SQLite busy timeout + retry。長時間失敗ならHALT |
...
| GMO Private GET | 20 req/s | account/open orders/executions |
| GMO Private POST | 20 req/s | order/change/cancel。ただし内部上限はさらに低くする |
...
1. SQLiteの最終 `equity_snapshots`、open positions、open ordersを読む。
...
| `HARD_HALT` | 全停止 | daemonの緊急クローズ、手動確認のみ |

DD -15% は `HARD_HALT`。CLI失敗連発やデータstale長期化は `SOFT_HALT`。
...
GMO側では銘柄ごとにmaker/taker feeが異なるため、ハードコードしない。
...
- `limitPrice < bestAsk` ならmaker候補。後続tradesで `trade.price <= limitPrice` が発生したら、キュー係数に応じて部分約定。
...
```kotlin
package me.matsumo.fukurou.paper
...
data class PaperSimulationContext(
    val tradeRule: me.matsumo.fukurou.domain.model.TradeRule,
...
[設計提案] 確定合格ラインに加え、実弾移行前の推奨ゲートを置く。ただしこれは確定事項の追加条件ではなく、安全な運用判断のための推奨である。
...
- exposure上限超過を拒否する。
- 残高不足・レート超過・tool call超過を拒否する。
- `adjust_stop` がSTOPを緩める場合に拒否する。
...
1. GMOコイン現物APIのネイティブ逆指値（`executionType: STOP`/`OCO`）は対応済み（確認済み）。
   - [確定] v1は取引所ネイティブの逆指値を第一の保護とし、bot停止中も建玉を守る。サーバーサイドSTOPはバックアップ。
   - [対応] 少額実弾前に、トリガ時の成行スリッページ・資金拘束・部分約定挙動を実機で検証する。

2. GMO APIの現物symbol表記の最終確定。
   - [仮定] 内部は `BTC_JPY`、APIは `GmoSymbolMapper` で変換。
...
| 保護ストップ作動前にプロセス停止 | 損失拡大 | 取引所ネイティブ逆指値を常駐(第一)＝bot停止でも取引所側で作動。サーバーサイドSTOPはバックアップ。Docker restart＋起動時復旧 |
...
4. Postgres移行
   - SQLiteで十分に検証後、並行読み取りやWebUI強化が必要ならPostgresへRepository実装を追加する。

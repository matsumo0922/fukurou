...
[確定] 定期15分発火だけで96起動/日になる。Step1スパイクでは、1起動あたりの実測token・所要時間・tool call数を記録し、daemonのcadence設計とサブスク枠見積もりの入力にする。
...
5. 発注が必要な場合のみ trade.place_order / close_position / update_protection を呼ぶ。
...
6. entry intentと `protectiveStopPriceJpy` をDBへatomicに保存する。

...
false-suppression proxy は distinct episode 単位で集計する。客観的 material change または invalidation を false、未知の入力を unknown、未完了 episode を pending、客観差分なしで terminal に到達した episode を valid とする。rate は `false / (false + valid)` であり、pending と unknown は分母から除外する。

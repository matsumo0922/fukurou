...
sequenceDiagram
    ...
    MCP->>APP: phase/invocation/hash付き判断提出(FD 5)
...
| PostgreSQL MCP接続情報 | fixed MCP launcher                        |         いいえ | root-only fileからsealed FDでMCP processへ渡す |
...
- fixed PID 1 は provider / MCP child の process group を start gate 前に確定し、launcher proxy の cancel half-close 後に既知 group と UID 10002 / 10003 inventory を空にしてから固定 acknowledgement を返す。exact peer PID と start ticks が一致する待機中 proxy だけを acknowledgement 前に除外する。runner は acknowledgement と proxy group exit の両方が揃う場合だけ終了を証明し、欠落時は未確認として次の LLM launch を fail closed にする。

...
評価 API/report は requested period と epoch lifecycle との積集合である effective period を別々返す。requested period がepoch開始前を含む場合は `PARTIAL_LIFECYCLE` と effective days を返し、積集合が空な場合は `EMPTY_LIFECYCLE` と null の effective date を返す。benchmark の trade population、prior PnL aggregate、market candle window は effective period で計算し、requested 90D を 90D の実績として表示しない。

...
- `ShellProcessRunner` は provider / MCP child を自身の setsid process group で起動し、`kill -TERM/-KILL -<pgid>` 後の `/proc` 走査で group が空になったことを確認する。process が自然終了した場合はこの確認だけで終了を証明する。timeout / cancellation による forced kill は、`/proc` 走査後に発生し得る遅延 fork を排除できないため終了未確認のままとし、recovery blocker を登録して次 tick の recovery scan に委ねる。

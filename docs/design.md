...
評価 API/report は requested period と epoch lifecycle との積集合である effective period を別々返す。requested period がepoch開始前を含む場合は `PARTIAL_LIFECYCLE` と effective days を返し、requested 90D を 90D の実績として表示しない。

...
- CLI process が認証失敗らしい stdout / stderr を出した場合、runner は exit code にかかわらず `RUNNER_PHASE_COMPLETED.details.authFailureSuspected = "true"` と login runbook の warn log を追加する。fail-closed の no-trade 動作は変えない。

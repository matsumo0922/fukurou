...
system prompt v1.14 は、直近 `no_trade_conditions_ja` の entry trigger / invalidation 分類、goalpost-moving 禁止、高 volatility 時の risk-based sizing と ATR based STOP、ブレイク水準への STOP entry intent 検討を要求する。`knowledge_get_recent_lessons` の SafetyFloor 拒否は Falsifier verdict と別 layer として読み、同一 setup / thesis を再提案する場合は現在の MCP evidence と prior intent の客観差分を示す。申告勝率・expected R・TP・STOP を threshold に合わせるだけの変更は改善と扱わず、差分を示せない場合は NO_TRADE とする。full EXIT は persisted canonical thesis が一意な場合だけ同じ thesis の risk-increasing order cancel と position close を原子的に実行する。EV gate は resting LIMIT entry を maker fee(rebate) と保護 exit 側 taker fee / slippage reserve で評価し、板を跨ぐ LIMIT / MARKET / STOP entry を taker fee と entry / exit 両側の market slippage reserve で評価する。既定 NO_TRADE、STOP 必須、ナンピン禁止、最大 drawdown 停止、exposure 上限は維持する。
...
sequenceDiagram
    ...
    participant APP as App submission gateway
...
`llm_runs` は `invocation_id` を primary key とし、`mode`、`symbol`、nullable な daemon `trigger_kind`、`status`、epoch millis の `started_at` / `finished_at`、`error_message`、`terminal_cause`、開始時 runtime config の `runtime_config_version_id` / `runtime_config_hash` を保存する。`terminal_cause` は `NORMAL_COMPLETION`、`NO_TRADE`、`SAFETY_DENIED`、`TIMED_OUT`、`RUNNER_FAILED`、`CALLER_CANCELLED`、`RESTART_INTERRUPTED`、`LEGACY_UNCLASSIFIED` の安定 code を持ち、RUNNING 行だけ null とする。LLM provider は phase ごとの `command_event_log` に残すため run-level には持たない。`stdout_path` / `stderr_path` も持たない。Claude の `error_message` と stdout / stderr は redaction と truncate 後に保存し、stdout / stderr は historical usage fallback に使う。Codex は起動境界の例外 message / path を永続化せず、`llm_runs.error_message` は固定文言のままとする。runner phase audit（`command_event_log`）は、`DefaultLlmOutputParser.parseCodex()` が primary category の先勝ち解決とは独立に追跡する `authEvidenceObserved`（既知の認証 evidence 文言 `CODEX_KNOWN_AUTH_EVIDENCE_TEXTS` を stdout/stderr のいずれかで一度でも観測したか）が false の場合に限り、次の2経路のいずれかで redaction と truncate 後の stdout / stderr を保存する。1つ目は process lifecycle 経路で、failure category が `PROCESS_EXIT` / `PROCESS_TIMEOUT` / `CLEANUP`（CLI プロセス自体の起動・終了・timeout・cleanup に起因し、adapter が Codex の出力から failure を一切検出していない場合）に限る。2つ目は issue #295 で追加した output-interpreted 経路で、failure category が `OUTPUT_CONTRACT` / `RATE_OR_SESSION_LIMIT` / `QUOTA_EXHAUSTED` / `UNKNOWN_PROVIDER_FAILURE`（Codex の出力テキストを解釈して決まるカテゴリ）の場合に、`authEvidenceObserved == false` であれば記録する。`authEvidenceObserved` は既知文言との一致判定に過ぎず、未知の secret やローテーション後の token 等、既知パターンに一致しない内容の不存在を証明するものではない（特に `OUTPUT_CONTRACT`（非 JSON raw garbage のケース）と `UNKNOWN_PROVIDER_FAILURE` はこの限界の影響を受けやすいカテゴリとしてユーザー確認済みで受容している）。それ以外（`AUTHENTICATION`、`authEvidenceObserved == true` の場合、lifecycle category と adapter-derived failure が同時に存在する複合ケース、成功時）は、固定 category と安全な例外 class 名だけを保存し、raw output・token・path・prompt は残さない。manual launch warning と standalone runner stderr でも元例外の message、path、stack trace は出さず、固定 category と安全な例外 class 名だけを出す。

`NO_TRADE_EXIT` と HARD_HALT 拒否の payload は provider による分岐を持たない。例外 message は `QueueSnapshotDiagnostics.PERSISTABLE_MESSAGES` の固定文言と完全一致した場合だけ `message` キーへ残し、一致しない cause は例外 class 名だけを残して `message` キー自体を出さない。判定を prefix ではなく完全一致にするのは、guard が任意の `Throwable` を cause として受けるためである。prefix 判定では外部由来の例外が同じ接頭辞を持つ message を構築でき、その後続へ secret を混入できてしまう。完全一致であれば、通過する値はコードが定義した定数そのものに限られる。
...
Evaluation Report Console は `POST /evaluation/reports/jobs` で 7D / 30D / 90D または最大365日の CUSTOM complete calendar days を手動 snapshot 化し、`GET /evaluation/reports/default` と `GET /evaluation/reports/revisions` から scope ごとの immutable revision を読む。manual job、revision number、共通 LLM launch reservation または rejection は `risk_state FOR UPDATE` と共通 quota policy を使う同一 transaction で確定する。report は Reflection を含む全 active invocation を blocker とし、trading decision 用 headroom を残す。rejected job も履歴へ残り、report request rate は shared quota と異なる `REPORT_RATE_LIMIT` として記録する。admission、stage、terminal、recovery は append-only job event に保存し、409 / 429 は `Retry-After` と blocker identity を返す。生成は browser request から分離した async job とし、server-side LLM invocation は MCP と tool を無効にした JSON-only report prompt を既存 `LlmInvocationAuditor` 経由で実行する。実行 request と revision metadata は同じ reasoning effort 値を使い、process failure、cleanup failure、usage/cost、redacted error と reservation は同じ invocation ID を使う。external candle provenance を取得した後、internal facts は PostgreSQL の単一 REPEATABLE READ transaction で読み、全 chart 値、fact metadata、provenance、usage、entity別除外数を含む integrity 全体を canonical typed snapshot row に materialize してから LLM を起動する。truncated input と external failure は fail closed にする。revision は snapshot materialization 時点の `inputAsOf`、単一 snapshot ID、canonical `inputHash`、typed facts、claim validation、Historical Outcome Ridge を同じ artifact に保持する。artifact/job/pin/reservation の成功または失敗は terminal transaction で確定し、起動時に残る非 terminal report は `FAILED_PROCESS_INTERRUPTED` へ回収する。validator は prose を書き換えず、claim ごとに verified / conflict / insufficient evidence / fact missing / not verifiable を返す。

immutable revision は bot realized / buy-and-hold / no-trade の日次 equity、forecast probability と realized win rate の calibration bin、setup × market regime の performance cell、exclusion・missing R・LLM usage/cost coverage、realized R distribution を型付き visualization facts として保持する。各 chart は `chartIndex` から fact ID と source ID へ到達でき、unit、sample count、sample state、authority、table fallback を同じ snapshot から表示する。データがない場合は空の系列や `INSUFFICIENT_SAMPLE` / `MISSING` / `UNAVAILABLE` を保持し、固定 demo 値で補完しない。
...
既存の `byModel` と `unattributedTokenPhaseCount` は provider が `usage.modelUsages` に記録した observed model だけを根拠にし、`configuredModel` から token attribution を補完しない。
...
API list-price 相当額と coverage は response 全体と provider 別に返し、observed model 別 token response には投影しない。
...
runner は claim 後の reservation lifecycle を所有し、success、failure、cancellationを claimant token 付き conditional finish で terminal にする。claim commit の応答が不明な場合は child process を起動せず、独立supervisorへ登録してから同じ token で read-after-unknown を行い、未解決中は readiness と新規 admission を fail closed にする。claim heartbeat と100件keysetのbounded current-process DB scanはbootstrapに依存せず継続し、DB復旧後もrestartを待たずに回収する。recovery tickはscan開始前に新規起動とreadinessを閉じ、monotonic clockで一度だけ作る5秒のpage deadlineをscanと全entity transactionへ渡す。scanの実行中であること自体はterminal submissionを止めず、submission gateはscanが失敗した場合だけ閉じる。各callはcancellationを確認し、残り750ms未満では開始しない。DB statementは残り時間からrollback reserve 250msを引いた`statement_timeout`、それ未満かつ最大2秒の`lock_timeout`、ceil秒のquery timeout、残り時間以下のnetwork timeoutを直前にarmし、commit直前にも再armする。PostgreSQL 16に存在しない`transaction_timeout`には依存しない。pageのstable transition lock内でentityを1件ずつ独立transactionにし、成功直後にそのclaimのblockerとfenceを解除する。後続失敗時も先行commit済みentityは解放済みで、cursorとscan healthは進めず、retry scanがterminal rowを除外して残件だけを収束させる。UPDATE 0またはmutation応答不明時はreservationとattempt auditをexact readbackする。別terminalはblocker/fenceを解放してpage処理を継続し、RUNNING identity変更はblocker/fenceを維持して継続する。欠損・片側commit・矛盾・readback失敗はpending attemptを保持してpageを失敗させ、cursorを進めない。`CLAIMED`は時刻にかかわらずactive blockerであり、recoveryは`claimedAt + H + G`到達、heartbeat `3I` miss、同tokenの`NO_CHILD_STARTED`またはrootと全descendantの終了を証明した`PROCESS_TREE_EXITED` fenceのANDとし、観測heartbeatをWHEREへ含める。終了証明が不確実な場合はfenceを設定せずadmissionを閉じる。各invocationのreservation、対応するRUNNING `llm_runs`、fence metadata付きauditをimmutable entity scope tokenと同一transactionで終端・記録する。runner finish、sweeper、bootstrapは`status=RUNNING`のconditional transitionを共有してterminal reasonを上書きしない。applicationとruntimeのPostgreSQL poolは30秒の初回connection retry windowに加えて終了判定時に進行中のJDBC connect/validation 1回を待ち、初期化後のpool acquisitionは500msでfail closedする。whole-run hard timeout はclaim commitを絶対deadlineの起点とし、PRE_FILTER / PROPOSER / FALSIFIER の各1回、TERM/KILL grace、terminal persistence timeoutから checked arithmetic で導出し、起動時auditに各componentとH/I/3Iをsecretなしで保存する。各material/child境界では同tokenのRUNNING状態とhealthを再確認する。

current-process recoveryはentityごとに固定attempt UUIDを割り当て、reservation mutationとaudit IDを同じtransactionへ保存する。transactionの自動retryは行わず、reservation identity・terminal値とaudit identity・時刻のexact readbackでcommitを確定する。同じattempt UUIDの生存期間全体でinitial mutationと明示retryの最大2回だけmutationを開始できる。serviceはpending attemptごとにprocess-local shared atomic retry permitを作り、initial recoveryと次tickのreconcileへ同じinstanceを渡す。元のRUNNING identityとaudit不在をexact readbackでき、entity scope tokenを取得した後かつreservation mutation SQLの直前にpermitを不可逆取得できる場合だけ1回redriveする。token lockやconnection取得でmutation境界へ到達しないfailureとreadback failureではpermitを未消費のまま保持する。permit取得後にcancellationやtimeoutでrepository結果が破棄されても取得状態は共有instanceに残る。消費後も未commitなら以後はread-only照合だけを行い、第三mutationを開始しない。未確定attemptは次tickのscan前に同じtransition lockで照合する。

application shutdown は current-process recovery worker を cancel 後6秒上限で termination まで待ち、timeout を成功として扱わない。resource cleanup は後続 close を継続し、最初の failure へ別 instance の後続 failure を suppressed として保持する。同じ failure instance の再送出は self-suppression せず、cleanup 結果を observer へ1回通知する。primary error reporter が失敗した場合も JDK logger が cleanup failure と reporter failure を記録する。
...
entry 系 decision は、server-owned の opportunity episode、NFKC と whitespace 正規化済み thesis、sorted setup tags、typed invalidation predicates、order geometry、episode anchor からの価格帯を含む material-state identity を decision と trade intent に同時保存する。episode 開始時の価格変化閾値を episode row に固定し、途中の runtime config 変更では再解釈しない。manifest または bounded projection が欠ける decision / intent は post-boundary の coverage failure として append-only に保存し、coverage の分母へ残す。resting entry だけが存在する daemon tick は full LLM run を起動せず、共有 market quote と global trading lock を使う deterministic maintenance へ分岐する。この分岐は order の cancel、replace、fill、TTL、order type を変更しない。

maintenance は typed invalidation を `VALID / INVALIDATED / UNKNOWN_DATA` で評価し、quote や identity が不足した場合は fail closed で unknown fact を保存する。fill、typed invalidation、non-TTL cancel、TTL 後の material-state change は episode を冪等に閉じる。TTL expiry 単独では episode を閉じない。

false-suppression proxy は distinct episode 単位で集計する。客観的 material change または invalidation を false、未知の入力を unknown、未完了 episode を pending、客観差分なしで terminal に到達した episode を valid とする。複数 resting order の maintenance は tick ID と order ID の組を監査単位にし、1件でも入力不明なら tick 全体を unknown とする。rate は `false / (false + valid)` であり、pending と unknown は分母から除外する。
...
`close_ratio` は `0 < close_ratio <= 1.0` の decimal string とし、省略時は全量 close として扱う。決済数量は残 position 数量に `close_ratio` を掛けた値を取引所の `sizeStep` へ床丸めし、丸め後の数量が 0 の場合は拒否する。決済後の残量が `sizeStep` 未満になる場合は dust 防止のため全量 close に昇格する。`REDUCE` はこの単一 position close を使う。full `EXIT` は別の `SAME_THESIS` ledger primitive を使い、対象 position の persisted intent から canonical thesis を解決した後、同じ thesis の risk-increasing `OPEN` / `PENDING_CANCEL` BUY order の status CAS、同じ thesis の全 open position の最新 locked size による fill 計算、close / account 更新を共通 lock order の単一 `READ_COMMITTED` transaction で行う。lock 待ち中に先行 fill が commit した場合は、lock 取得後の statement snapshot で position / order を再解決する。linkage の欠損、null、複数、矛盾、stale target は mutation 前に typed failure とし、trade-group equality を thesis の代用にしない。
...
| Claude/Codex認証     | CLI runtime container の `llm-auth` volume | はい、CLI実行に必要 | exchange secretとは隔離               |
| PostgreSQL app接続情報 | daemon/Reconciler                         |         いいえ | secret値は環境変数で渡す                   |
| PostgreSQL MCP接続情報 | MCP subprocess config（renderer literal env） |         いいえ | 既存app envの`DB_PASSWORD`をCodex/Claude MCP server専用envへ渡す |
...
- CLI login state は production compose の `llm-auth` volume に保存し、container restart / redeploy で維持する。per-run config/session と cleanup quarantine は `/run/fukurou/llm-homes` tmpfs に置く。WebUI System は `/ops/llm-auth` で状態を表示し、`/health` / `/health/ready` の意味には混ぜない。
...
- Codex は device auth flow でログインし、WebUI に token/code 入力欄を出さない。永続 home は auth source としてだけ使い、renderer は per-run `CODEX_HOME` に auth file を copy して config/session を run 後に削除する。
...
- CLI provider failure は pinned version adapter が structured code から分類する。`AUTHENTICATION`、`RATE_OR_SESSION_LIMIT`、`QUOTA_EXHAUSTED`、`OUTPUT_CONTRACT`、timeout、process exit、cleanup、unknown を区別し、英語メッセージの部分一致は使わない。failure の raw output は保存せず、安全な provider code と adapter version を監査する。
- `ShellProcessRunner` は provider / MCP child を自身の setsid process group で起動し、`kill -TERM/-KILL -<pgid>` 後の `/proc` 走査で group が空になったことを確認する。process が自然終了した場合はこの確認だけで終了を証明する。timeout / cancellation による forced kill は、`/proc` 走査後に発生し得る遅延 fork を排除できないため終了未確認のままとし、recovery blocker を登録して次 tick の recovery scan に委ねる。recovery scan は blocker が指す reservation が DB 上で terminal になり、`finished_at` から `hardTimeout + processTerminationGrace` が経過した時点で blocker を解除し、admission を再開する。判断根拠は DB の終端事実に限り、claimant token が一致しない blocker や `finished_at` を欠く blocker は解除しない。
- LLM invocation audit は semantic submission、process exit、artifact cleanup を `details.terminal` に独立投影する。repository 完了済みの risk-reducing decision は後続 timeout / cleanup failureでも `COMMITTED` のまま保持し、repository shutdown race は `UNKNOWN` とする。既存 status / exit / provider / usage fields は維持し、deterministic event producer は terminal projection を持たない。
- execution admission が gate するのは、新規 LLM 起動、runner による発注、および app-owned submission gateway 経由の terminal submission である。MCP server は独立 process として起動されるため process-local な admission health へ到達できず、その read-only tool call は admission の gate 対象ではない。read-only tool call の抑止は tool allowlist、tool call budget、manifest 有効期限、`HARD_HALT`、global trading lock が担う。
- submission gateway は `SUBMIT_FALSIFICATION` と、`EXIT` / `REDUCE` / `NO_TRADE` 以外の action の `SUBMIT_DECISION` を gate する。exposure を増やさないことが実装で保証できる action だけを免除し、`ADJUST_PROTECTION` は免除しない。take-profit のみを動かし、既存 take-profit との単調性も上限も課されないため exposure を延ばせるからである。gate の判定は repository 呼び出しの直前に行い、判定から commit までの区間は atomic ではない。
- gate の条件は、admission blocker の有無と、同一 invocation で完了した child の `UNCERTAIN` 履歴の有無の論理和とする。前者は process 全体の健全性を、後者は同一 run の過去 phase が終了を証明できなかったことを表す。`UNCERTAIN` による recovery blocker の登録は one-shot 全体の終了時に行われるため、run の途中で後続 phase を止めるには process tree termination registry を直接参照する。registry の entry は、単一 phase の呼び出し元では phase 監査完了時に、複数 phase をまたぐ呼び出し元では run 終了時に解放する。後者の解放は終端状態の永続化より前に行い、DB 障害時の残留を防ぐ。
- submission gate の条件は、admission blocker の 3 集合が空であることに加えて、直近に完了した recovery scan が成功していることとする。periodic recovery scan の正常な実行中は submission を拒否しない。scan は周期的に走るため、実行中まで fail-closed にすると run ごとに 1 回だけ起きる submission がその窓に当たって失われる。一方 scan の実障害は stale claim を発見できない状態を意味するので fail-closed のままとする。新規起動と `/health/ready` は従来どおり scan 実行中も fail-closed にする。
...
[確定] polling `ProtectionReconciler` は起動時に full reconcile pass を実行する。production WebSocket path は loop 開始前に global trading lock 下で bounded `HARD_HALT` cleanup を実行し、connect failure、begin-session failure、session 終了後の backoff、market event、periodic maintenance でも同じ cleanup を再試行する。各 attempt は `ALL_OPEN_RISK` の atomic readback から始まり、flat / order-only なら ticker なしで収束し、position が残る場合だけ現在価格を取得する。REST `TickSnapshot` は app 観測時刻 `observedAt` と exchange source 時刻 `sourceTimestamp` を別に保持する。REST ticker を close execution authority にする直前に source timestamp の parse 成功、現在時刻から5秒以内の鮮度、未来方向5秒以内の clock skew を検証し、simulation context の orderbook 取得後に ledger mutation 直前の現在時刻でも同じ source timestamp を再検査する。不正・stale・過度な未来時刻なら mutation を作らず `HARD_HALT / UNKNOWN` を維持する。realtime trade event の `TickSnapshot` は exchange 時刻と受信時刻を分離するが、causal event そのものを authority とするため REST freshness gate を適用しない。これはクラッシュ復元と同じ safety authority を使い、main push ごとの deploy 方針は変えない。`/health/ready` はDB接続だけでなく `lastTransportActivityAt` と `lastMaintenanceAt` の鮮度、接続状態、未回復gapを確認し、保護ループが止まっている状態をreadyにしない。`lastTradeAt` は正常無音を許容するため readiness の必須条件にしない。
...
[確定] WebSocket listener は complete trade message の process clock observation を先に固定し、`paper_market_event_receipts` へ durable receipt を commit してから application queue へ event を渡す。receipt commit は receipt ID、global admission ordinal、normalized payload hash、DB に保存された canonical socket observation 時刻を返し、listener はその4値を event に付けて `receivedAt` を canonical 時刻へ揃える。同一 `(session_id, source_sequence)` と同じ normalized payload hash は既存 receipt の ID、ordinal、original canonical timestamp を返し、rowとordinalを増やさない。payloadが異なるduplicateとreceipt persistence failureはtyped infrastructure gapとしてqueue/fillへ渡さない。receipt transaction は session-scoped shared advisory lockと同一source identityのduplicateを直列化する短いadvisory lock、`paper_market_admission_ordinal_seq`、receipt 1 rowだけを使い、`market_data_sessions` rowとledger rowをlockしない。既存historyのbackfill/dedupeとreceipt pruningは行わず、receiptは最低365日保持できるschemaとする。

[確定] PostgreSQL の realtime resting BUY は、作成 transaction が session-scoped exclusive advisory lock、connected `market_data_sessions` row、risk state、paper account、positions、orders の順で authority を取得し、既存の admission ordinal unique index を使った global maximum を nullable `market_eligible_after_admission_ordinal` に保存する。event application は session row を ledger row より先に lock し、最後に event が主張する exact receipt を再読する。receipt ID、session、source sequence、ordinal、payload hash、canonical socket timestamp、`receivedAt`、recomputed normalized payload が一致し、receipt ordinal が order boundary より大きい場合だけ risk-increasing fill を評価する。`market_eligible_from` は互換列として残すが entry ordering authority には使わない。null legacy boundary、missing receipt、または contradictory evidence は current-session entry を `market_data_gap` で取り消し、SafetyViolation FK detail は作らない。同じ transaction の mark、risk-reducing STOP / virtual TP、cursor advance は継続し、旧 session の BUY を新 session へ rebind しない。

...
| action | 必須tool |
|---|---|
| `ENTER_LONG` | get_ticker, get_candles(5m/1h/1d), get_orderbook, get_trades, get_symbol_rules, get_balance, get_positions, preview_order, knowledge_get_recent_lessons, fresh_falsifier_approval |
| `ADD_LONG` | 上記 + current_position + pyramid_status |
| `UPDATE_PROTECTION` | get_positions, calc_indicator(ATR), preview_protection_update |
| `CLOSE` / `REDUCE` | get_ticker, get_orderbook, get_positions, preview_close |
| `HOLD` | get_ticker, get_positions, get_trailing_floor |
...
1. `submit_decision` が `ENTER_LONG` / `ADD_LONG` を受けた場合、entry intentを `PENDING_FALSIFICATION` としてDBへ保存する。
...
```json
{
  ...
  "properties": {
    ...
    "closeRatio": { "type": "string", "default": "1.0" },
    "reasonJa": { "type": "string", "minLength": 20 }
  }
}
```

`closeRatio` は `0 < closeRatio <= 1.0` の decimal string とし、省略時は全量 close として扱う。決済数量は残 position 数量に `closeRatio` を掛けた値を取引所の `sizeStep` へ床丸めし、丸め後の数量が 0 の場合は拒否する。決済後の残量が `sizeStep` 未満になる場合は dust 防止のため全量 close に昇格する。

...
3. 次の catalog query が1行を返し、`indisvalid`、`indisready`、`indislive` がすべて true であることを確認する。index 名の存在だけを成功条件にしない。
...
5. catalog validity の確認後に新 image を deploy する。resolver はこの3 flag が揃うまで無効で、TTL capture だけが動作する。

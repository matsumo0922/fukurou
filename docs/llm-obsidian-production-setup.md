...
- Standard snapshot が作れない run は material manifest の `STANDARD_CONTEXT` reason に typed stage を保存する。`ACCOUNT_SNAPSHOT` から `RUN_MANIFEST_PERSISTENCE` の具体的 stage を確認し、generic な snapshot unavailable や例外 message を原因として扱わない。

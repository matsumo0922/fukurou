...
- Runner は CLI process が認証失敗らしい stdout / stderr を出した場合、exit code にかかわらず `RUNNER_PHASE_COMPLETED.details.authFailureSuspected = "true"` と login runbook の warn log を残す。これは運用上の発見シグナルであり、runner は no-trade に fail closed する。

...
- Standard snapshot が作れない run は material manifest の `STANDARD_CONTEXT` reason に typed stage を保存する。`ACCOUNT_SNAPSHOT`、`MARKET_DATA_SOURCE` から `RUN_MANIFEST_PERSISTENCE` までの具体的 stage を確認し、generic な snapshot unavailable や例外 message を原因として扱わない。caller cancellation は typed stage ではなく infrastructure event として扱う。
- `RISK_REDUCTION_ONLY` は `processFailureKind` がある runを `risk_reduction_only_process_failed`、typed process fact がなく decision もない runを `risk_reduction_only_missing_decision`、gateway 検証済み `NO_TRADE` decision を `NO_TRADE_DECISION` として区別する。`cleanupFailed`、`cliErrorReported`、`authFailureSuspected` は process hard failure と混同せず併記する。

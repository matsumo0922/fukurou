...
2. deploy 後の container 内で Claude / Codex CLI にログインする。
...
CLI の refresh token が失効または revoke された場合は、同じ手順で再ログインする。通常の access token 更新は CLI が自動で行う。login state は `llm-home` volume に保存されるため、container restart / redeploy では残る。`docker volume rm fukurou_llm-home` のように volume を削除すると Claude / Codex の login state も消える。

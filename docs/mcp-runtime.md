...
- `submit_decision` と `submit_falsification` はMCP roleでDBへ書かず、phase manifest ID / invocation ID / effective invocation hashに束縛されたowner-only Unix socketを通してapp processの`LlmDecisionSubmissionGateway`へ提出する。gatewayはapp roleの既存decision repositoryを呼び、`RISK_REDUCTION_ONLY`ではEXIT / REDUCE / ADJUST_PROTECTION / NO_TRADE以外をrepository呼び出し前に拒否する。socketはnormal、timeout、cancel、CLI/MCP crashのいずれでもphase終了時にclose/unlinkする。

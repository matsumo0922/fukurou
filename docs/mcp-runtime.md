...
- bridge は image build 時に同じ MCP fat jar から生成した snapshot で `initialize` / `tools/list` に即答する。JVM backend の handshake が終わるまで `tools/call` などの実処理 request は順序を保って queue し、`ping` は queue に入れず即答する。backend 起動失敗、handshake 失敗、backend exit は非 0 exit として fail-closed 経路に乗せる。

...
`daemon.enabled` は desired state の唯一の正本で、WebUI `/app/controls` の start / stop も versioned draft、typed validation、activate を通る。supervisor は `STARTING` / `RUNNING` / `STOPPING` / `STOPPED` / `DEGRADED` の observed state を持ち、process restart なしで worker を起動・停止する。通常 stop は新規 tick を止めて in-flight run の完了を待ち、`runner.perRunTimeout + 30秒` を超えると cancel して監査する。start failure は desired=true を維持し、5秒から最大5分までの bounded exponential backoff で再試行する。HARD_HALT 中の start は runtime config を変更する前に拒否する。

active runtime config の `daemon.*` が変わると supervisor は graceful drain 後に process 起動時 config の daemon section だけを差し替えて worker を再構築する。runner / safety / paper など daemon 以外の active 値は worker rebuild へ混ぜず、process restart まで process applied config として固定する。`GET /ops/daemon` は active full config、process applied full config、daemon applied section の version / hash、restart 要否、scheduler signal、last launch / skip、next heartbeat、in-flight run、無音警告を返す。`daemonAppliedConfig.hash` は `daemon.*` section だけの content hash である。無音は `daemon.flatHeartbeatInterval + daemon.pollInterval` を超えて scheduler signal がない場合に警告し、desired=false または意図的停止は警告しない。`/health/ready` は daemon status で変化しない。

...
- `submit_decision` と `submit_falsification` はMCP roleでDBへ書かず、phase manifest ID / invocation ID / effective invocation hashに束縛されたowner-only Unix socketを通してapp processの`LlmDecisionSubmissionGateway`へ提出する。既存terminal requestは1 frameのままversioned evidence bundle fieldを持つ。現行binaryはcode-owned activationを常にfalseに固定し、collectorは`DISABLED`を送り、tool response、link、coverage rowを作らない。additiveな`llm_tool_evidence`、`llm_terminal_evidence_links`、`llm_decision_phase_evidence_coverage`、`llm_tool_evidence_activation_boundaries`はfoundationとして存在するが、fresh install、upgrade、旧imageへのrollbackのいずれでもactivation boundary rowは作らない。gatewayはapp roleの既存decision repositoryを呼び、`RISK_REDUCTION_ONLY`ではEXIT / REDUCE / ADJUST_PROTECTION / NO_TRADE以外をrepository呼び出し前に拒否する。socketはnormal、timeout、cancel、CLI/MCP crashのいずれでもphase終了時にclose/unlinkする。

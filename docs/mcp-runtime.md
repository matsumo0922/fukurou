...
- event 条件: `FUKUROU_ECONOMIC_EVENT_BLACKOUTS_UTC` の active window で 1 回だけ起動する。
- holding 状態: open position / open order が DB にある場合は 3 時間 cadence。
- 起動予算: 1/hour、4/day。event も heartbeat も同じ予算を消費する。
...
| `FUKUROU_LLM_MAX_INVOCATIONS_PER_DAY` | `4` | runner / daemon が audit / DB から数える 24 時間起動上限 |
...
runner 上限も保守側の override だけを許可する。総 tool call は 48 以下、trade 系 tool call は 3 以下、timeout は 180 秒以下、起動数は 1 回/時・4 回/日以下だけを受け入れる。tool call 上限は同じ `decision_run_id` の `command_event_log` と MCP server instance 内 counter を合算して、Proposer / Falsifier の phase をまたいで強制する。上限超過時は tool error を返し、no-trade audit を残す。

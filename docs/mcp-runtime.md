...
Reflection Runner の週次 PromptCandidates は `REFLECTION` trigger として同じ reservation / cap を使う。reflection の RUNNING 予約は trading trigger を `CONCURRENT_INVOCATION` で塞がない。fresh な trading RUNNING 予約、HARD_HALT、1時間 cap の残り 1 回以下、24時間 cap の残り 4 回以下では PromptCandidates の LLM を呼ばず、status note だけを残す。
...
| `FUKUROU_REFLECTION_PROMPT_CANDIDATE_PROVIDER` | `CLAUDE` | 週次 PromptCandidates に使う LLM provider |
| `FUKUROU_REFLECTION_PROMPT_CANDIDATE_TIMEOUT_SECONDS` | `60` | 週次 PromptCandidates の LLM timeout。120 秒以下だけ許可 |
| `FUKUROU_REFLECTION_PROMPT_CANDIDATE_MAX_ATTEMPTS` | `2` | 同じ週の PromptCandidates LLM 最大試行回数 |

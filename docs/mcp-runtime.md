...
LLM invocation は in-flight run を supersede しない。scheduler は trigger を既存 priority で選んだ後に active reservation を確認し、阻まれた trigger と blocker の invocation ID / trigger kind / key を `DAEMON_TRIGGER_SKIPPED` に記録する。current-process recovery worker は bounded DB scan を heartbeat interval ごとに実行し、stale `AVAILABLE` を回収する。stale `CLAIMED` は `H + G` 経過、heartbeat `3I` miss、同一tokenの `NO_CHILD_STARTED` または `PROCESS_TREE_EXITED` fenceがすべて成立する場合だけconditional `FAILED`へ遷移する。対応するRUNNING `llm_runs`とclaim token fingerprint / state / heartbeat / termination fenceを含む`LLM_INVOCATION_RECOVERED`はreservationと同じtransactionで終端・記録する。新heartbeatが先に書かれた場合はUPDATE 0とし、terminal確認までadmissionを自動再開しない。bootstrapはsingle-instanceの旧process generation終了確認後だけCLAIMEDを`RESTART_INTERRUPTED`として補助回収し、invocationごとに`LLM_INVOCATION_RECOVERED`を残す。`llm_runs.terminal_cause` は status と直交する machine-readable な終了理由であり、legacy terminal row は `LEGACY_UNCLASSIFIED` として読む。

...
手動 `OneShotRunnerMain` は daemon の `llm_launch_reservations` を通らないため、daemon の `CONCURRENT_INVOCATION` guard からは見えず、schedulerと同時実行しない。productionでdirect canaryが必要な場合は、`daemon.enabled=false`をactive化してprocessを再起動し、scheduler worker不在、RUNNING reservation 0、RUNNING `llm_runs` 0、`OneShotRunnerMain` process 0を確認してからisolated disposable canaryを実行する。canary後に同じ4条件を再確認し、`daemon.enabled=true`をactive化して再起動する。通常のmigration完了確認ではdirect runnerを実行せず、gate ONは自動テスト、production smokeはreservationを共有するschedulerと`POST /ops/trigger`を正本とする。runner preflight の rolling cap は reservation 優先規則を使い、手動実行は reservation のない audit fallback として daemon 起動と相互に数える。preflight で拒否された手動実行も phase / NO_TRADE audit を残し、window 外へ出るまで 1 起動として数える。手動実行も開始時に active runtime config snapshot を解決し、その version id / hash を decision run audit へ残す。

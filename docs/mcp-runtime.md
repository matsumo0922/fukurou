...
`fukurou-mcp` runtime と Docker 配線の正本メモ。
...
- Docker image には Ktor 用 `/app/app.jar`、fukurou MCP 用 `/app/fukurou-mcp-all.jar`、MCP handshake bridge 用 `/app/fukurou-mcp-bridge.mjs`、bridge snapshot 用 `/app/fukurou-mcp-tools.json`、standalone GMO Coin MCP 用 `/app/gmo-coin-mcp-all.jar` を同梱する。
...
- container の entrypoint は Ktor のまま。production compose の daemon / CLI runtime は `node /app/fukurou-mcp-bridge.mjs --snapshot /app/fukurou-mcp-tools.json ${mcpJarPath}` を MCP stdio 子プロセスとして起動し、bridge が裏側で `java -jar /app/fukurou-mcp-all.jar` を起動する。
- bridge は image build 時に同じ MCP fat jar から生成した snapshot で `initialize` / `tools/list` に即答する。`FUKUROU_MCP_ALLOWED_TOOLS` がある場合、bridge は client に返す `tools/list` snapshot を phase ごとの allowlist に絞る。JVM backend の handshake が終わるまで `tools/call` などの実処理 request は順序を保って queue し、`ping` は queue に入れず即答する。backend 起動失敗、handshake 失敗、backend exit は非 0 exit として fail-closed 経路に乗せる。
...
```sh
docker build -t fukurou:mcp-runtime .
docker run --rm --entrypoint /bin/sh fukurou:mcp-runtime -lc 'test -f /app/fukurou-mcp-all.jar && test -f /app/fukurou-mcp-bridge.mjs && test -f /app/fukurou-mcp-tools.json && test -f /app/gmo-coin-mcp-all.jar'
docker run --rm --entrypoint /bin/sh fukurou:mcp-runtime -lc 'node -e "const s=require(\"/app/fukurou-mcp-tools.json\"); if (!s.toolsListResult.tools.length) process.exit(1)"'
```

CLI から production image 内の stdio MCP を登録する場合の command / args:

```json
{
  "command": "node",
  "args": [
    "/app/fukurou-mcp-bridge.mjs",
    "--snapshot",
    "/app/fukurou-mcp-tools.json",
    "/app/fukurou-mcp-all.jar"
  ]
}
```

ローカル checkout の jar を直接使う場合は `command` を `java`、`args` を `["-jar", "mcp/build/libs/fukurou-mcp-all.jar"]` に差し替える。
...
```json
{
  "mcpServers": {
    "fukurou-mcp": {
      "command": "node",
      "args": ["/app/fukurou-mcp-bridge.mjs", "--snapshot", "/app/fukurou-mcp-tools.json", "/app/fukurou-mcp-all.jar"],
      "env": {
...
Claude Proposer は既定 allowlist で `get_trade_intent` などの read tool と `submit_decision` を許可し、`place_order` は許可しない。runner は同じ allowlist を MCP server env の `FUKUROU_MCP_ALLOWED_TOOLS` に短い tool 名で渡す。bridge は client に返す `tools/list` snapshot をこの allowlist で絞り、JVM MCP server は `tools/call` 時に allowlist 外 tool を no-trade audit 付きで拒否する。bridge の snapshot drift 突合は raw snapshot と backend の raw `tools/list` を比較するため、phase allowlist による恒常 warning は出さない。`FUKUROU_PROPOSER_ALLOWED_TOOLS` / `FUKUROU_FALSIFIER_ALLOWED_TOOLS` は `mcp__<server-name>__<tool-name>` 形式だけを受け付け、Claude / Codex の組み込み tool 名は許可しない。Claude runner は常に private `--mcp-config` と `--strict-mcp-config` を渡し、`--tools ""` で built-in tool を無効化する。MCP を使わない reflection は空の MCP config と `--bare` で起動し、on-disk project 設定や auto-discovery 由来の tool を読ませない。Codex Falsifier は `intent_id` だけを入力として受け取り、Proposer の理由・thesis・ナラティブは prompt/env に渡さない。Falsifier process env は非 secret の CLI 起動 env、run env 5 変数、`FUKUROU_FALSIFIER_INTENT_ID`、renderer が生成した `CODEX_HOME` に限定し、DB 接続は MCP server config file にだけ明示する。GMO API key/secret や LLM credential env は CLI process / MCP server env のどちらにも渡さない。CLI 認証は production container 内の `llm-home` volume に保存し、auth file path を app env として渡さない。
...
| `FUKUROU_MCP_JAR_PATH`                                | `mcp/build/libs/fukurou-mcp-all.jar` | one-shot runner が bridge または JVM 直起動に渡す MCP fat jar。production compose は `/app/fukurou-mcp-all.jar` を指定する            |
| `FUKUROU_MCP_SERVER_NAME`                             | `fukurou-mcp`                        | Claude / Codex に登録する MCP server 名                                                                        |
| `FUKUROU_MCP_SERVER_COMMAND`                          | `java` / production compose は `node`   | MCP server 起動 command。production compose は bridge を起動する                                                        |
| `FUKUROU_MCP_SERVER_ARGS`                             | 未指定 / production compose は bridge args | MCP server 起動引数。local で未指定時は `-jar FUKUROU_MCP_JAR_PATH`、production compose は bridge path / snapshot path / `${mcpJarPath}` を渡す |
...
| `FUKUROU_MCP_ALLOWED_TOOLS`                           | runner が設定                           | bridge が client に返す snapshot と MCP server の call guard で許可する short tool 名。通常は手動設定しない                          |

...
child start lease と recovery terminal decision は同じ claimant token 単位で直列化する。Linux process group は root process の通常終了時も TERM/KILL と member 0 の確認を行い、終了証明が不確実な claim の health blocker は reservation terminal persistence だけでは解除しない。

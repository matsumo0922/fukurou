...
`:mcp-gmo-coin:smokeStdio` は standalone GMO Coin MCP fat jar を stdio 子プロセスとして起動し、SDK client へ渡す前の raw stdout を全行 JSON として parse しながら Public `get_ticker` tool を呼び出す。stderr は診断出力として許容し、stdout は JSON-RPC 専用 channel として検証する。

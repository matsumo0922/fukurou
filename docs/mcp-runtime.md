  ...
  "mcpServers": {
    "fukurou-mcp": {
      ...
      "env": {
        ...
        "FUKUROU_SYSTEM_PROMPT_VERSION": "system-prompt-v1.13",
...
paper 約定、SafetyFloor、GMO Public REST の rate-limit / retry / timeout、GMO Public WebSocket の connect timeout / message stale timeout / reconnect backoff、runner 上限、daemon cadence / trigger、Reflection Runner、LLM model override は WebUI `/app/config` の Runtime group で管理する。取引 symbol / mode、GMO Public REST / WebSocket URL、Obsidian vault path、LLM command template、MCP command / args / allowlist、persistent home は deployment boundary として env で管理する。runtime の現在値と code-owned default は `GET /ops/runtime-config` または WebUI で確認する。
...
- WebUI System は `/ops/llm-auth` の CLI auth と `/health/ready` の WebSocket connection、last received time、gap/recovery を表示する。readiness は DB、startup recovery、接続、active gap、message freshness、periodic safety maintenance の freshness がすべて正常な場合だけ ready になる。
...
- WebSocket 待機中も `ProtectionReconciler` は periodic maintenance と trade message の stale 期限を独立に管理する。最後の受信時刻から `gmoPublic.websocketStaleTimeout` を超えた接続は `MESSAGE_STALE` gap として閉じ、有効な `gmoPublic.websocketReconnectBackoff` で再接続する。受信buffer overflow、subscription拒否、decode失敗もgapとして監査する。REST tick は HARD_HALT、kill criterion、ATR trailing の保守だけに使い、resting entryを約定させない。

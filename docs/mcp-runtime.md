...
- WebUI System は `/ops/llm-auth` の CLI auth と `/health/ready` の WebSocket connection、last received time、gap/recovery を表示する。readiness は DB、startup recovery、接続、active gap、message freshness がすべて正常な場合だけ ready になる。

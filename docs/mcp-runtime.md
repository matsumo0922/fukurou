...
LLM invocation は in-flight run を supersede しない。scheduler は trigger を既存 priority で選んだ後に active reservation を確認し、阻まれた trigger と blocker の invocation ID / trigger kind / key を `DAEMON_TRIGGER_SKIPPED` に記録する。`CLAIMED` reservationは時刻にかかわらずterminalまたはfence-safe recoveryまでactive blockerであり、`AVAILABLE` / `NOT_REQUIRED`だけをinclusiveなstale cutoffで判定する。current-process recovery worker は100件のkeyset DB scanをheartbeat intervalごとに実行し、scan statementを2秒、tick全体を5秒に制限する。stale `AVAILABLE` を回収し、stale `CLAIMED` は `H + G` 経過、heartbeat `3I` miss、同一tokenの `NO_CHILD_STARTED` またはprocess tree全体の終了を証明した`PROCESS_TREE_EXITED` fenceがすべて成立する場合だけconditional `FAILED`へ遷移する。Linux productionではLLM CLIを専用process groupで起動し、group全体へのTERM/KILL後に`/proc`上の非zombie memberが0になった場合だけ終了証明を成立させる。process groupを利用できない環境や列挙が不確実な場合はfenceを設定しない。1 pageのreservationはstable orderで順次処理し、各invocationのimmutable population scopeから取得したtokenを使う。同じinvocationのreservation、対応するRUNNING `llm_runs`、claim token fingerprint / state / heartbeat / termination fenceを含む`LLM_INVOCATION_RECOVERED`は同一transactionで終端・記録する。新heartbeatが先に書かれた場合はUPDATE 0とし、terminal確認までadmissionを自動再開しない。bootstrapはsingle-instanceの旧process generation終了確認後だけCLAIMEDを`RESTART_INTERRUPTED`として補助回収し、invocationごとに`LLM_INVOCATION_RECOVERED`を残す。`llm_runs.terminal_cause` は status と直交する machine-readable な終了理由であり、legacy terminal row は `LEGACY_UNCLASSIFIED` として読む。

...
- Standard material は account、ticker、candles、orderbook、indicator projection、canonical hash、material persistence、run manifest persistence の境界を typed stage で保持する。失敗時の minimal material は stage 名だけを `STANDARD_CONTEXT` の missing reason に保存し、raw market response と例外 message を保存しない。orderbook provenance は `GMO_PUBLIC_ORDERBOOK`、上位件数は `MaterialOrderbookSummary.levelLimit` を正本とする。

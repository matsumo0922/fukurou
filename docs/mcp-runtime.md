...
- `market_data_ingress_sessions` はapp-owned private tableであり、`fukurou_mcp` roleのtable inventoryには含めない。durable ingress foundationはproduction call graphから非接続で、MCPのread/write面を増やさない。
- `submit_decision` と `submit_falsification` はMCP roleでDBへ書かず、phase manifest ID / invocation ID / effective invocation hashに束縛されたowner-only Unix socketを通してapp processの`LlmDecisionSubmissionGateway`へ提出する。現行binaryはcode-owned activationを常にfalseに固定し、evidence providerをbindせず既存version 1のfield setとserialized bytesを維持する。activation用version 2 bundleは同じ1 frameと128 KiB上限を使い、app gatewayはactivationとversion/fieldの不一致を拒否する。additiveな`llm_tool_evidence`、`llm_terminal_evidence_links`、`llm_decision_phase_evidence_coverage`、`llm_tool_evidence_activation_boundaries`はfoundationとして存在するが、fresh install、upgrade、旧imageへのrollbackのいずれでもactivation boundary rowやevidence rowを作らない。gatewayはapp roleの既存decision repositoryを呼び、`RISK_REDUCTION_ONLY`ではEXIT / REDUCE / ADJUST_PROTECTION / NO_TRADE以外をrepository呼び出し前に拒否する。socketはnormal、timeout、cancel、CLI/MCP crashのいずれでもphase終了時にclose/unlinkする。
...
- 内部のcoverageは指定した14日範囲をraw manifest/evidence JSONなしのaggregate queryで読み、terminal decision、RUNNING、terminal no-decision、legacy、structural incompleteを別件数にする。prune repositoryはactivation boundary以後かつ満30日を経過したrootだけを`< cutoff`で選び、最大500 rootをchildからparentへ1 transactionで削除する。未配線の`LlmAuditMaintenanceWorker`はcoverage失敗後もpruneを試行し、prune失敗時は後続batchを止め、1回最大10 batchでtyped reportを返す。captureとruntime schedulingはdisabledであり、このworkerは`Application`、`TradingRuntime`、schedulerから参照されない。

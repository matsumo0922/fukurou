...
PID 1 は provider / MCP child の PGID を parent 側で確定してから start gate を開き、launcher proxy の TERM を request socket の half-close として受け取る。cancel 時は全既知 job group と UID 10002 / 10003 の process inventory を TERM、deadline 後は KILL し、adopted child を reap する。job root が先に正常終了した場合もPID 1がadopted childをreapし、process group memberが消滅した後だけjob slotを解放してlauncher proxyへ終了statusを返す。保存済み response socket の `SO_PEERCRED` PID と `/proc` start ticks が一致する proxy だけを acknowledgement 前の inventory から除外する。job group と非除外 inventory が空になった後だけ固定 acknowledgement を返す。runner は acknowledgement と proxy group の終了を両方観測した場合だけ process exit を証明済みにし、acknowledgement が欠ける場合は未確認として launch admission を停止する。
...
`scripts/mcp-credential-isolation-check` は exact production image の fixed LLM/MCP launcher、FD 3/4 bootstrap、app-owned gatewayを接続したFD 5、local GMO fixture、least-privilege role を通して Proposer/Falsifier の required 16 calls を実行する。加えてRISK_REDUCTION_ONLYの6 actionをFD5から提出し、EXIT / REDUCE / ADJUST_PROTECTION / NO_TRADEの受理とENTER / ADD_LONGの拒否を確認する。受理する4 actionは固有のinvocation IDを使い、`RISK_REDUCTION_ONLY` authorityが4件すべて`COMPLETED`かつ4件の固有ID集合と一致することを同じDBで検証する。別manifest IDと別effective hashの実gateway requestを拒否し、schema version 1のactivation boundary、`TERMINAL_BUNDLE_CAPTURED` coverage、evidence/linkの同数性・phase所属・連続ordinalを同じDBで検証する。fixed Codex wrapperのcommand template / revision / image fingerprint / versionがphase manifestへ保存された値と一致することもDBから確認する。FD 6以上が閉じていること、MCP roleのopportunity token、全許可列INSERT、direct close UPDATE、terminal evidence 4 tableへのwriteが拒否されてもgateway submitとopportunity protocol row保存が成功することを同じimageで確認する。`--reuse-image <exact-image>`はbuildxなしで既存imageを検査し、省略時は指定tagへlocal buildする。macOS/GNU statを判別し、Docker capabilityは`CAP_` prefixと配列順を正規化した集合で比較する。`llm_runs`とrequired MCP viewsのreadiness後にrole SQLを適用し、llm-agentが作成したnested treeをproduction cleanup配線で削除できることも検証する。UID/setuid/capability/secret read境界に加え、raw provider stdout/stderr、gateway stdout/stderr、application/container log、per-run manifest、tool audit export、data-only dumpをcleanup前に取得する。raw marker、独立segment、JSON/TOML escape、URL percent encode、base64/base64url、hexを全artifactで検査する。coverage不足、readiness failure、空audit、空dump、scan failureはpassにしない。

同じ script の `--qualification --runs 3 --reuse-image <repository@sha256:digest>` は immutable repository digest を
一度だけ照合し、foundation と pinned Claude/Codex acceptance を同じ image に束縛する。
...
output parser 経由で検証する。Claudeはwrite annotation付き`submit_decision`、Codexはproduction承認ロジックで`submit_falsification`を呼び、acceptance prompt はprefixed MCP tool ID、fixture server instructionsはrequired short tool名を明示する。Codex configはMCP serverをrequiredにし、fixture record pathだけをMCP subprocessへ明示転送する。fixture call record と response marker を独立に検証する。primary failure codeとcleanup statusは独立して報告する。raw provider output は harness 外へ出さず、失敗時は0700のlocal harness artifactを表示pathに保持する。deploy pipeline は qualification と acceptance smoke を自動実行しない。operator が CLI/provider compatibility を確認するときに exact digest を指定して手動実行する。1-run container timeoutは720秒、3-run qualificationは2160秒である。

...
- full run は Proposer 起動前に secret と raw orderbook を含まない immutable material-state manifest を invocation ID ごとに1件保存する。material schema v2 は ticker、bounded candle / indicator、上位10 levelから算出した orderbook summary、同一 ledger snapshot の account / position / open order、source provenance / truncation を typed field で保持する。required ticker と存在する candle の timestamp / OHLC は正の有限値と価格関係を検証し、不正値を欠損へ変換しない。#185 identity hashとは別のsnapshot content hashがtyped bundle全体を束縛し、run manifestは同じhashを参照する。PostgreSQLではroot、run、materialのinvocation scopeとhashを同じtransactionで検証し、片方だけまたはcross-root bundleを残さない。account snapshot は `REPEATABLE READ READ ONLY` 1 transactionで読み、positionはSQL `LIMIT 33`から32件、open orderは`LIMIT 65`から64件をmanifestへ保存する。in-memory runtimeはbroker read fallbackを持たず、risk stateとledgerのwriter-visible boundaryが同一の場合だけ読む。entry decision / intent の identity は MCP input から受け取らず、この manifest と提出済み TradePlan / geometry から server が生成する。
...
- Standard material は account、market data source configuration、ticker、candles、orderbook、indicator projection、canonical hash、material persistence、run manifest persistence の境界を typed stage で保持する。失敗時の minimal material は stage 名だけを `STANDARD_CONTEXT` の missing reason に保存し、raw market response と例外 message を保存しない。caller cancellation は typed stage に変換せず、infrastructure event として上位へ伝える。orderbook provenance は `GMO_PUBLIC_ORDERBOOK`、上位件数は `MaterialOrderbookSummary.levelLimit` を正本とする。
- `RISK_REDUCTION_ONLY` の hard gate は `START_FAILURE`、`TIMEOUT`、`NON_ZERO_EXIT` の typed process fact だけである。CLI/auth heuristic と cleanup failure は audit signal として残り、typed process fact がなければ gateway 検証済みの `EXIT`、`REDUCE`、`ADJUST_PROTECTION`、`NO_TRADE` を妨げない。decision 不在は `risk_reduction_only_missing_decision` と `RUNNER_FAILED` に分離し、valid `NO_TRADE` decision として評価しない。

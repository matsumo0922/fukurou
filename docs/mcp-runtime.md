...
FUKUROU_LLM_MAX_INVOCATIONS_PER_HOUR=7
FUKUROU_LLM_MAX_INVOCATIONS_PER_DAY=120
...
- event 条件: `FUKUROU_ECONOMIC_EVENT_BLACKOUTS_UTC` の active window、価格急変、STOP 接近、paper entry fill を既存 reservation / cap 経路で評価する。経済イベントは同じ active window で 1 回だけ起動する。
...
手動 `OneShotRunnerMain` は daemon の `llm_launch_reservations` を通らないため、daemon の `CONCURRENT_INVOCATION` guard からは見えない。手動実行は daemon を停止してから行うこと。runner preflight の rolling cap は reservation 優先規則を使い、手動実行は reservation のない audit fallback として daemon 起動と相互に数える。手動実行も開始時に active runtime config snapshot を解決し、その version id / hash を decision run audit へ残す。
...
| `FUKUROU_LLM_MAX_INVOCATIONS_PER_HOUR` | `7` | runner が reservation / legacy audit から数える 1 時間起動上限 |
| `FUKUROU_LLM_MAX_INVOCATIONS_PER_DAY` | `120` | runner / daemon が reservation / legacy audit から数える 24 時間起動上限。15 分 cadence の 96 起動に event burst 24 起動分を加えた hard cap |
...
| `FUKUROU_LLM_TRIGGER_ENTRY_FILL_ENABLED` | `true` | paper entry fill trigger の有効化 |
| `FUKUROU_LLM_TRIGGER_ENTRY_FILL_COOLDOWN_SECONDS` | `600` | paper entry fill trigger の cooldown。daemon poll interval 以上だけ許可 |

...
`daemon.enabled` は desired state の唯一の正本で、WebUI `/app/controls` の start / stop も最新 active version を基準にした versioned draft、typed validation、expected active version 照合付き activate を通る。Config 画面の generic activate / rollback も daemon section を変更する場合は同じ control serialization、HARD_HALT / STOPPING guard、監査、reconcile を通る。supervisor は `STARTING` / `RUNNING` / `STOPPING` / `STOPPED` / `DEGRADED` の observed state を持ち、process restart なしで worker を起動・停止する。通常 stop は監査などの副作用待ちより先に新規 tick を止め、in-flight run の完了を待つ。process 起動時の `runner.perRunTimeout + 30秒` を超えると cancel し、cancel 後の終了待ちも固定上限で打ち切る。非協調 worker が残る間は新 worker を起動せず `DEGRADED` とし、5秒から最大5分までの bounded exponential backoff で終端を再確認する。この再確認中は同じ `STOPPING` / `DRAIN_TIMED_OUT` 監査を繰り返さず、状態遷移時だけ記録する。start failure と active runtime config の一時的な解決失敗も同じ上限の bounded exponential backoff で自動再試行する。HARD_HALT 中の start は runtime config を変更する前に拒否する。

active runtime config の `daemon.*` が変わると supervisor は graceful drain 後に process 起動時 config の daemon section だけを差し替えて worker を再構築する。runner / safety / paper など daemon 以外の active 値は drain timeout と worker runtime を含めて process 起動時の値へ固定する。`GET /ops/daemon` は active full config、process applied full config、daemon applied component の `component` / `sourceVersionId` / component hash、restart 要否、scheduler signal、last launch / skip、next heartbeat、in-flight run、無音警告を返す。合成 worker config は実在する単一 version に対応しないため、判断監査では runtime config version ID を設定せず、合成 content hash を記録する。無音は `daemon.flatHeartbeatInterval + daemon.pollInterval` を超えて scheduler signal がない場合に警告し、desired=false または意図的停止は警告しない。意図的停止の運用理由は完了済み desired state 監査から process restart 後も復元する。`/health/ready` は daemon status で変化しない。

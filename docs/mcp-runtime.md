...
- event 条件: `safety.economicEventBlackouts` の active window、価格急変、STOP 接近、paper entry fill を既存 reservation / cap 経路で評価する。FOMC は Federal Reserve 公式 calendar（https://www.federalreserve.gov/monetarypolicy/fomccalendars.htm）の会合最終日 14:00 `America/New_York` を UTC へ変換し、前後 60 分を `fomc-` ID で保存する。calendar の `validThrough` は保存せず、FOMC window の最大終了時刻から導出する。active calendar が空、不正、期限切れの場合は runtime 全体と readiness を止めず、新規 entry だけを fail closed にする。close、cancel、protection update、holding position の管理、ProtectionReconciler は継続する。draft / validate / activate は malformed JSON、重複 ID、不正 window、FOMC 0 件、future window 0 件を拒否する。経済イベントは同じ active window で 1 回だけ起動する。

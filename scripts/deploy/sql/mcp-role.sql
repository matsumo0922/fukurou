...
FROM (VALUES
    ...
    ('evaluation_exclusions'), ('infrastructure_gaps'), ('safety_violations'),
...
SELECT format('GRANT SELECT (invocation_id,mode,symbol,trigger_kind,status,started_at,finished_at,error_message,terminal_cause,runtime_config_version_id,runtime_config_hash) ON TABLE public.llm_runs TO %I', :'mcp_role') \gexec
SELECT format('GRANT SELECT (id,account_epoch_id,trade_group_id,mode,symbol,side,status,opened_at,closed_at,size_btc,average_entry_price_jpy,current_price_jpy,current_stop_loss_jpy,current_take_profit_jpy,unrealized_pnl_jpy,unrealized_r,pyramid_add_count,highest_price_since_entry_jpy,lowest_price_since_entry_jpy,decision_run_id,tool_call_id,client_request_id,llm_provider,prompt_hash,system_prompt_version,market_snapshot_id,market_data_session_id,market_eligible_after_sequence) ON TABLE public.positions TO %I', :'mcp_role') \gexec

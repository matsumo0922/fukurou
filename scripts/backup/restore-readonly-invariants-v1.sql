...
SELECT CASE
  WHEN COUNT(*) = 1
    AND BOOL_AND(id = 1)
    AND BOOL_AND(mode = 'PAPER')
    AND BOOL_AND(initial_cash_jpy >= 0)
    AND BOOL_AND(cash_jpy >= 0)
    AND BOOL_AND(btc_quantity >= 0)
    AND BOOL_AND(total_equity_jpy >= 0)
  THEN 0 ELSE 1
END
FROM public.paper_account;

-- Exactly one runtime configuration is active and every value references a version.
SELECT CASE WHEN COUNT(*) FILTER (WHERE status = 'ACTIVE') = 1 THEN 0 ELSE 1 END
FROM public.runtime_config_versions;
...
WHERE num_nonnulls(account_epoch_id, execution_semantics_version, runtime_config_hash) NOT IN (0, 3);

...
- `standardStage`: standard material の失敗境界。`ACCOUNT_SNAPSHOT`、`MARKET_DATA_SOURCE`、`TICKER`、`CANDLES`、`ORDERBOOK`、`INDICATOR_PROJECTION`、`CANONICAL_HASH`、`MATERIAL_PERSISTENCE`、`RUN_MANIFEST_PERSISTENCE` のどこで fail-closed したかを示す。`<none>` は typed failure stage が保存されていないことだけを示し、material row の有無や失敗 stage の推定には使わない。
- `rroProcess`: `RISK_REDUCTION_ONLY` child process の typed fact。`START_FAILURE`、`TIMEOUT`、`NON_ZERO_EXIT` は hard failure、`cleanup=true`、`cli=true`、`auth=true` は独立した audit signal として読む。typed process fact がない `risk_reduction_only_missing_decision` は infrastructure outcome、保存済み `NO_TRADE` decision は strategy decision として区別する。
...
- API endpoint / パラメータ: `fukurou/src/main/kotlin/me/matsumo/fukurou/EvaluationRoutes.kt`、`AGENTS.md` の Placeholder API 一覧

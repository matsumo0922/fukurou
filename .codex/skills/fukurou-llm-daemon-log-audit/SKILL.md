...
- `standardStage`: standard material の失敗境界。`ACCOUNT_SNAPSHOT`、`TICKER`、`CANDLES`、`ORDERBOOK`、`INDICATOR_PROJECTION`、`CANONICAL_HASH`、`MATERIAL_PERSISTENCE`、`RUN_MANIFEST_PERSISTENCE` のどこで fail-closed したかを示す。`<none>` は standard material が保存された状態であり、失敗 stage の推定には使わない。

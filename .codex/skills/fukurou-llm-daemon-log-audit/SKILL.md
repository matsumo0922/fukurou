...
- `/ops/daemon` の `activeConfig`、`appliedConfig`、`daemonAppliedConfig` はそれぞれ active full config、process 起動時の full config、worker が採用した `daemon.*` component の identity。`daemonAppliedConfig.sourceVersionId` は daemon section の取得元、`daemonAppliedConfig.hash` は daemon section だけの hash。合成 worker config は単一 version と偽装しない。`restartRequired=true` は daemon 以外の active 値が process restart 待ちであることを示す。
- runtime config の一時的な解決失敗は `DEGRADED` / `RUNTIME_CONFIG_UNAVAILABLE` と `nextRetryAt` を確認する。bounded retry で valid active snapshot へ戻るため、意図的停止と混同しない。

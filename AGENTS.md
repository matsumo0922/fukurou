...
### 要領よく進める（bias to action）

agent は完璧主義と過剰スコープに寄りやすい。issue の受け入れ条件を満たす最小の変更で完了とし、以下の停止条件を守る。

- 修正の完璧さより issue の本旨を優先する。根治が本旨から外れて diff が肥大するなら、最小修正 + 既知の限界の明記で完了とし、残りは報告する。
- 実装中に気付いた別の問題・改善・リファクタは diff に入れず、完了報告に列挙する。
- 不具合の再現・観測・計測は 3 試行を上限とする。再現できなければ「再現不能」と記録し、仮説ベースの最小修正または報告へ切り替える。条件を変えた実験を延々と続けない。
- 実装まで依頼されているタスクでは、調査・計測だけでターンやセッションを終えない。妥当な仮定を置いて実装へ進み、置いた仮定は報告に明記する。調査・計測だけを依頼された場合は結論の報告で完了とし、指示なしに修正へ移行しない。
- 同じファイルの読み直しや同じ計測の繰り返しで進捗が止まったら、その時点の結論をまとめて切り上げる。
- design skill / autopilot の設計は、受け入れ条件を満たす最小構成を第一案とする。拡張・汎用化・将来対応は設計本文に含めず、「やらないこと」または Next steps に 1 行ずつ列挙するだけにする。
- 新しい計測・監視・実験 harness は、それ自体が issue の受け入れ条件である場合だけ追加する。

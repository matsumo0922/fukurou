...
**最重要**: 本プロジェクトは個人が遊び/実験目的で開発しているシステムである。ゆえに、会社で用いられるような公のシステムでもなければ、他人の資金を運用する厳格なシステムでもない。個人で使うものなため、多くの実験的実装や方針転換が行われることを前提に issue に取り組まなければならない。設計を行う際も同様で、過度な堅牢性やセキュリティは必要ない。ユーザーが望むのは特定の目的を達成するために（issue）いかに「要領よく」実装を行うか、というところだ。小さな不具合やスコープ外の実装、必要以上の監視・計測に注力することのないように注意せよ。
